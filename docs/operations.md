...
`compile-artifacts` returns a launch checklist for every Dispatch it compiles: exact Dispatch ref/hash plus each unit, Dispatch task fragment, attempt, allowed output path, and required schema. The main agent registers only launches for which it received an external acknowledgement by calling `register-dispatch-launches`; it may submit several atomic batches in any order. `check-dispatch-launches` is read-only and derives started/not-started sets from immutable same-Run Lane Lifecycle records. It does not call, poll, stop, retry, or authenticate Codex tasks. Duplicate or conflicting identities fail closed, and downstream readiness, fan-in, conversion, gate, or reporting cannot silently bypass a Dispatch lane that has neither a launch registration nor a formal current-Run disposition. The main agent, not the Harness, chooses whether to wait, accept partial work, retry through current Task/Plan contracts, skip/cancel, or adapt the Plan.
