...
## G2.2 User Pre-candidate Confirmation Boundary

After G2.2 fan-in publication and before G2.3 synthesis, the caller must present the exact current fan-in and materialized pre-candidates to the user. Durable user-interest state is recorded only through `confirm-pre-candidates`, which appends a `pre_candidate_interest_confirmed` Decision with the exact fan-in ref/hash, every materialized concrete pre-candidate ref/content hash, the caller-attested user confirmation text, and the fixed disclosure that Harness identity verification is not available.

The only current actions are `proceed_with_selected`, `run_additional_discovery_same_scope`, and `stop_current_run`. `proceed_with_selected` requires one or more selected retained concrete pre-candidates; G2.3 may formalize only those selected refs. `run_additional_discovery_same_scope` records that the user selected none from this fan-in and allows the caller to use the ordinary Gap Snapshot, Adaptation Decision, Plan Revision, dispatch, and fan-in path for more discovery under the same confirmed Scope. `stop_current_run` records no selected refs and is the single action for both scope mismatch and no desire to continue this Run; a different market, language, decision scope, or adjusted Scope starts a new Run after the current Run is stopped. The latest same-fan-in append-ordered confirmation is the Manifest authority, so `confirmedAt` stays metadata only and repeat `follow-up-interest` refs remain orthogonal to selection.

`not_selected_current_run` is only a user-interest disposition for this Run. It does not rewrite fan-in retained/watchlist/rejected dispositions, delete source material, reject Evidence, lower confidence, or assert that no Evidence exists. A newer same-fan-in `pre_candidate_interest_confirmed` Decision is the authority for G2.3 entry; any latest action other than `proceed_with_selected` blocks G2.3 publication.
...
## Terminal Captured Substrate Inventory

Terminal report provenance may include `captured_substrate_inventory` when an honest non-completed terminal outcome (`failed`/`blocked`, `insufficient_evidence`, `cancelled`, or `deprioritized`) leaves same-Run Evidence Store records that were captured but never bound by a formal typed Evidence Artifact. The projection is Harness-derived from exact `startup_opportunity.evidence_store_record.v2` records, is omitted for completed terminal reports, and is only emitted when at least one unformalized record exists.

The inventory is audit-only. It preserves every exact `evidence/manifest.jsonl#ev_*` ref, canonical public URL or user-provided URI, research goal, `recorded_at`, `captured_not_formalized` status, mechanically derived unformalized reason, optional handoff binding, and the fixed `does_not_support_conclusions` boundary. It must not feed `sources`, `report_citations`, Evidence dispositions, coverage, Gate, ranking, confidence, recommendation ceilings, report statistics, source ordering, or Evidence sufficiency. Repeated raw/source identities remain separate items; any readable grouping must preserve every exact record ref and must not double-count independent records as readable sources.

User-visible rendering is field-specific and fail-closed: status, unformalized reason, conclusion boundary, and Evidence Store source kind are rendered through exhaustive current-schema localizers, while research goals and other caller/research prose remain literal identity. Decision Brief shows the inventory only as a bounded summary; the full report, audit appendix, and structured `report.json` retain every exact item. Multiline research goals are rendered as continuations of their owning Markdown list item so literal prose cannot establish report-section boundaries. The zh-CN user-view guard is bound to structured surfaces: the Harness-owned Gate diagnostic section in the audit appendix must exactly project localized rows from `gate_warnings`, while the brief and core report do not lexically scan free research prose. Free research prose, including decision questions, conclusion prose, source claims/inference, uncertainty reasoning, validation prose, freshness, limitations, and inventory research goals, is not scanned by keyword/path regex. The guard must not use substring positions, first/last Markdown fragment matches, field-by-field prose whitelists, global Evidence Store ref whitelists, or source/citation literal exemptions as authority.
