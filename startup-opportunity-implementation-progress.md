...
```text
用户当前明确指令 / current fixed ruleset
...
`PERSISTENT_MODE_POLICY_V1` 是 G2、G3、G4 及其返修流程的永久模式，不得在进入新 Gate、heartbeat 或 automation update 时恢复旧的逐 Gx.y task/acceptance/bookkeeping-only 流程：

- 每个 Gate 使用一个连续施工 worker。同一 worker 可跨该 Gate 的全部 Gx.y、内部修复和多个语义清晰原子提交；不逐 slice 停止验收或另开任务。
- worker 回合结束但 Gate 未完成时续派同一 task；只有原 task/worker 不可恢复时才创建替代 worker。任何时刻只有一个 local 写入者。
- 子切片运行专项与真正受影响回归，大型集成点运行全量测试。整个 Gate implementation exit candidate 完成后只创建一次 independent whole-gate regression。
- independent regression 保持独立且不修改 candidate。FAIL 后回到原 Gate worker 定向修复；candidate 改变后再创建 fresh independent regression。
- 取消逐 slice controller acceptance、逐 slice regression、bookkeeping-only/ledger-only task 与纯账本提交。账本只随正常实现、定向修复或 regression 提交更新。
...
定时中控每 10 分钟执行一次。普通 heartbeat 只读本文顶部、当前 Gate/active task、Git HEAD/status 和必要 diff；不完整重读长 RFC/账本，每次最多检查 active task 一次：
...
每次有效操作后，中控必须立即调用 automation update；只能刷新 automation 的“当前状态”区，不得删除、缩窄或改写 `PERSISTENT_MODE_POLICY_V1`。当前状态至少包含：
...
| `IMPLEMENTED_PENDING_WHOLE_GATE_REGRESSION` | 连续 Gate stream 中的 slice 已形成原子提交；不逐 slice 验收，等待整个 Gate exit candidate 的独立回归 |
...
| G1.R | Independent Assess Whole-Gate Regression | `DONE` | 端到端 assess fixture、正反结论、insufficient evidence、动态 adaptation、报告与恢复独立重放；中控独立重放并接受 |
...
| G2.2 | Discovery Lanes / Fan-in | `DONE` | accepted v9/v10 bytes 保持 immutable；candidate diversity fixture 与 exact pre-kill shared-unit adaptation binding 已通过 fixed-v5 boundary acceptance |
| G2.3 | Demand / Solution / Thesis Synthesis | `DONE` | v11/receipt v9 bytes 保持 immutable；general/industry/AI/hybrid candidate-to-thesis-to-report profile chain 已通过 fixed-v5 boundary acceptance |
| G2.4 | Enrichment / Business Engine / Comparison | `DONE` | v13/receipt v11 repair adapter 的 selected-solution AI gate、decision ceiling、Consistency v3 scan 与 Store/recovery compatibility 已通过 fixed-v5 boundary acceptance |
| G2.R | Independent Discovery Whole-Gate Regression | `DONE` | task `019fada4-6e70-7ce1-8b0e-2756ef0a9f20` 对 candidate `6d09cc7` 统一 PASS；H1-H14闭合 |

## G3 AI Bundle 施工切片

| 切片 | 内容 | 状态 | 主要退出条件 |
| --- | --- | --- | --- |
| G3.1 | AI Baseline / Reliability / Data Contracts | `DONE` | generic model、platform、open-source baseline；代表性 evaluation、failure/human boundary、data rights/feedback、portability |
| G3.2 | AI Economics / Commoditization / Trust Gates | `DONE` | product inference economics、bundle/substitution risk、capability half-life、adoption/trust、regulated-AI boundary |
| G3.3 | Mandatory Bundle Dynamic Coverage | `DONE` | versioned trigger驱动caller显式六维coverage；not_applicable/insufficient分离；缺失bundle限制结论；freshness continuation |
| G3.R | Independent AI Whole-Gate Regression | `DONE` | task `019fb188-5754-7430-859d-6f00303ea95f` 对 `e00f71333e115a37cfcaf45590943e975d408ab4` unified PASS，无未解决 P0/P1/P2 |
...
| G2.2 Scheme A directed contract repair | `ACCEPTED` | `76089efebb1b6714a1ce44603aefeacf5d5fa63a` | `be0b901345925504042eed7be337e848fe3595e2` |
...
| G2 fixed-v4 H14 archive portability repair candidate | `ACCEPTED` | `6d09cc7afbe2a3638f627f5e2fe4fe90412c683c` | `a0362b9cb2b1d013f7b0ec5614c2d5010ac91349` |
...
- G1.R 候选提交时只标记 `REGRESSION_CANDIDATE_PENDING_CONTROLLER_ACCEPTANCE`，没有自行关闭 G1、W4 或开放 G2。

### 中控独立验收

- 中控从 clean `main@a30c0967860bf31d26fc959c583f8c5b6b1b4caa` 独立核对 parent=`7dd044fb2ed6ca1ffdc79e633b239faf1940d20e`、branch=`main` 与空工作树，并接受该 G1.R 原子候选。
- 中控使用 Node.js `24.18.0` / npm `11.16.0` 独立重放 `npm ci`、lint、typecheck、`npm test` 182/182、schemas 67/63、doctor 179/179、`npm audit` 0 vulnerabilities、Git diff/check/status，以及冻结、真实性和禁止范围扫描，全部 PASS。
- 中控接受 final lineage、Audit/Matrix/Traceability/freshness/nested hash、serial/concurrent/sidecar-only report revision、report lock、双 ESM import order 与 recovery 的同 Gate 最小修复；该验收不改变上述真实性和外部验证边界。
- 因此 G1.R、G1 Concept Evidence Assessment 与 W4=`DONE`；G2 Opportunity Discovery 与 W5=`IN_PROGRESS`，但仅 G2.1=`READY`。G2.2-G2.R、G3-G4 保持 `NOT_READY`，本 bookkeeping follow-up 不创建或实现 G2.1。

## G2.1 Seed / Opportunity / Solution Space Maps Implementation Candidate

- 本独立施工以 clean `main@0525f08aeb127d031ed2e283be2dbd622d5e66c8` 为唯一起点，核对 parent=`a30c0967860bf31d26fc959c583f8c5b6b1b4caa` 与前序 `7dd044fb2ed6ca1ffdc79e633b239faf1940d20e`。施工期间未创建 subagent、worktree、Handoff、regression 或下一任务，未执行 push、amend、rebase、reset。
- 新增默认 schema bundle `7.0.0`（76 schemas / 71 document validators），以 versioned v8 contracts 发布 discovery Scope Frame v2、Seed Probe v1、Opportunity Space Map v1、Solution Space Map v1、Discovery Maps Policy v1、Artifact Envelope v8、Document Bundle v8 与 Research Publication Policy v4；既有 v1-v7 schema、bundle、policy bytes 不变。
- 四种 `general|industry_first|ai_first|hybrid` profile 均通过同 Run/current discovery Plan/single primary market-language/profile/path/ref/hash/revision/main-agent ownership 校验。Seed 只扩大检索入口，Evidence/audit refs 为空；一个 demand/task unit 只读取 Scope Frame，另一个 distinct `counter_evidence` unit 显式检验 user/job/scene/alternative。
- Opportunity Space Map 只保存 user/buyer/payer/decision-maker、JTBD/workflow/task profile、alternative/Baseline/workaround/friction/non-consumption/software leverage/state-context、buyer language、initial demand 与 disconfirming questions，不生成正式 Demand/Opportunity Thesis。Solution Space Map 在同一 solution-neutral demand boundary 下覆盖 ordinary software、platform-native、human/service-assisted、native app、mini program、mobile web/PWA、hybrid、AI-assisted 和 status quo；AI 只保存 capability frontier/failure/human review/data/evaluation/provider/open-source/capability half-life boundary，不选择 solution。
- v8 envelope/document bundle 使用 receipt v7；首次 map publication 必须以显式 closed three-map bundle 整体预验证，每个正式 path 继续使用既有 immutable/no-replace receipt。RunStore 为 discovery Plan 设置 `current_phase=discovery`，checkpoint/reopen 只从 validated on-disk envelope/temp/receipt 恢复。`validate-artifact`/`publish-artifact` 是唯一新增可用 surface；`discover` orchestration、G2.2 lanes/fan-in、G2.3 thesis、G2.4 enrichment/comparison/report 与 G3-G4 继续 fail closed。
- G2.1 专项 33/33 PASS：覆盖四 profiles、24 个 closed negative cases、seed-independent/counterfactual、solution neutrality、Seed/Evidence/score/probability boundary、same-Run/locale/profile/ref/path/hash/revision/producer、baseline/status-quo/delivery breadth、AI capability-only boundary、source/audit/limitations、three-map first publication、idempotent/conflicting replay、v7 receipt drift、map temp crash recovery、checkpoint/reopen、G2.2 adapter block 与 `discover` unavailable。
- 冻结 Node.js `24.18.0` / npm `11.16.0` 下 `npm ci` PASS（18 packages，0 vulnerabilities）；lint/typecheck PASS；`npm test` 215/215；`validate:fixtures` 117/117；Store 11/11、fault 10/10、recovery 11/11；schemas 76/71；doctor `g2.1` 全部 checks PASS；`npm audit` 0 vulnerabilities。
- `package-lock.json` SHA-256 仍为 `5271d765b9d1818ccace29aab770c629704a470c7c6a5827de07ddd4db68682e`，仓库只有一个 lockfile；旧 schema/policy/toolchain 路径相对 `0525f08` 零 diff。fixture 全部标记 `SYNTHETIC_ONLY_NOT_EVIDENCE`，没有 public URL 或 user material，不伪造 Evidence、quote、市场数据或 validation success；没有 network/LLM/subagent/external-validation 执行入口。
- 残余风险：Store bundle 先整体预验证，再按 path 写独立 receipt；若进程在已完成一个 map、尚未开始下一个 map 之间退出，reopen 会因三-map cardinality fail closed，需要调用方显式重放同一 bundle，不会从 chat 或 task completion 合成缺失 map。当前 recovery fixture 覆盖 matching v7 receipt + temp 的 interrupted map publication，不把该边界表述为跨文件事务。
- 本候选只把 G2.1 标记为 `EXIT_CANDIDATE_PENDING_INDEPENDENT_REGRESSION`。G2 Opportunity Discovery 与 W5 仍为 `IN_PROGRESS`；G2.2-G2.R、G3-G4 仍为 `NOT_READY`；本任务未开放或创建任何后续任务。

## G2.1 Independent Whole-Slice Regression Candidate

本独立回归以 clean `main@178df38c343003730c23d05ae57bf968bfef9bec` 为唯一起点，核对 candidate parent=`0525f08aeb127d031ed2e283be2dbd622d5e66c8`、branch=`main` 与空工作树。完整读取 RFC、本文、`AGENTS.md`、`$startup-opportunity` Skill，以及 opportunity-discovery、artifact-contracts、research-kernel、lane-catalog references；使用 `rg` 独立建立 schema/policy/validator/Store/publication/recovery/fixture/test/CLI/Skill/RFC 索引，并审查 candidate 的完整 45-file diff。未依赖实现任务口头结论，未创建 subagent、worktree、Handoff 或下一任务，未 push、amend、rebase 或 reset。

### 独立审查与 inter-map 对抗性回归

- 独立复核四 profiles、Seed/no-Evidence/no-score/no-probability boundary、seed-independent 与 distinct counter-evidence unit、solution-neutral Opportunity Space Map、九类 Solution Space Map/AI capability-only boundary，以及 same-Run/current Plan/mode/market/language/profile/ref/hash/path/revision/producer/source/audit/limitations fail-closed contract。v8 envelope/document bundle、Discovery Maps Policy、research-publication v4、bundle v7 与 receipt v7 的 exact binding 均由 schema、closed policy tuple、validator 和真实 Store publication/reopen 共同固定。
- implementation candidate 已覆盖 explicit three-map first publication、single-map rejection、immutable/no-replace、exact replay/idempotency、conflicting replay 零覆盖、checkpoint、temp/receipt crash recovery、receipt drift、G2.2+ adapter block 与 `discover` unavailable；本回归未发现需要修改 production owning path 的新缺陷。
- 新增真实 child-process/filesystem regression：worker 先让生产 `ArtifactStore.publishBundle` 完成完整 three-map prevalidation，再在排序后的第一张 map 与 v7 receipt durable publish 后以 exit code `86` 退出，此时第二、三张 map 尚未开始。`RunStore.load` 以 `reference.missing` 明确 fail closed，且完整 tree snapshot 证明 reopen 不写入、不覆盖第一张 map/receipt，也不从 chat/task completion 合成缺失 map。
- 调用方随后重放同一完整 bundle：已发布 map 返回 `idempotent_replay`，其 map/receipt bytes 保持不变，另外两张 map 确定性补全；checkpoint 后连续 reopen 均闭合且不再 recovery。由此接受“inter-map 边界不承诺跨文件事务，但 same complete bundle 可从 validated on-disk state 补全”的 G2.1 contract。

### 冻结环境与真实验证

全部 npm 命令使用 Node.js `24.18.0` 与 npm `11.16.0`，显式通过 `PATH=/opt/homebrew/Cellar/node@24/24.18.0/bin:$PATH npx --yes npm@11.16.0` 执行。

| 命令 | G2.1 independent regression 结果 |
| --- | --- |
| `npm ci` | PASS；18 packages installed，19 packages audited，0 vulnerabilities |
...
| `npm run test:faults` | PASS；10 tests，10 passed，0 failed/skipped/todo |
| `npm run test:recovery` | PASS；11 tests，11 passed，0 failed/skipped/todo |
| `npm run test:g2.1` | PASS；34 tests，34 passed，0 failed/skipped/todo；含真实 inter-map process exit/replay/reopen |
...
| `npm audit` | PASS；0 vulnerabilities |
| `git diff --check` | PASS；regression commit 前复核 |

### 冻结、真实性、范围与退出边界

- 相对 candidate parent `0525f08aeb127d031ed2e283be2dbd622d5e66c8`，toolchain、`package-lock.json`、`tsconfig.json`、schema v1-v7、bundle v1-v6 与既有 policy bytes 零 diff。`package-lock.json` SHA-256=`5271d765b9d1818ccace29aab770c629704a470c7c6a5827de07ddd4db68682e`；仓库仍只有一个 npm lockfile。
- G2.1 fixtures 明示 `SYNTHETIC_ONLY_NOT_EVIDENCE` / `SYNTHETIC` / `unvalidated_*`，public identifiers 与 user material refs 均为空，Evidence/audit refs 为空，不包含伪造 Evidence、URL、用户 quote、市场数据或 validation success。新增 worker 只操作临时本地 filesystem 和生产 Store；production owning paths 未出现 network、隐藏 LLM、agent/subagent dispatch 或 external-validation 执行入口。
- 本回归只新增 G2.1 filesystem test/worker 与本账本状态，不实现 G2.2 lanes/fan-in、G2.3 synthesis/thesis/freeze/dedupe、G2.4 enrichment/BusinessEngine/comparison/sensitivity/portfolio/report、G2.R 或 G3-G4，也不建设通用 Workflow Runtime、DAG DSL、daemon、UI 或 DB。
- 验证只证明 deterministic schema/policy/reference/publication/recovery contract，不证明任何真实 Opportunity、Demand、solution 选择、评分/概率、市场数据、Evidence 质量或 external validation success。未执行 network research、interview、landing page、deposit、广告、paid experiment、MVP test 或其他 external validation。
- G2.1 只标记 `REGRESSION_CANDIDATE_PENDING_CONTROLLER_ACCEPTANCE`；G2 Opportunity Discovery 与 W5 保持 `IN_PROGRESS`，G2.2-G2.R、G3-G4 保持 `NOT_READY`。本任务不开放 G2.2，不创建任何下一任务。

### 中控独立验收

- authoritative `wait_threads` 返回 cursor=`494f0650-027a-4022-b72f-d7889ae3a4c4:1`、revision=`1`、thread idle、latest turn completed。中控从 clean `main@b01b309bb477bd448803ed4ccfb63b2c9b40fc0b` 核对 parent=`178df38c343003730c23d05ae57bf968bfef9bec`、branch=`main`、空工作树、`git diff --check` 与 Git connectivity，并接受该 G2.1 regression 原子候选；connectivity 仅报告无害 dangling blob。
- 中控使用 Node.js `24.18.0` / npm `11.16.0` 独立重放 `npm ci`、lint 215 files、typecheck、`npm test` 216/216、schemas 76/71、fixtures 118/118、Store 11/11、faults 10/10、recovery 11/11、G2.1 34/34、doctor/verify 197 checks `ok=true`、`npm audit` 0 vulnerabilities，以及 Git status/diff/check，全部 PASS。
- 中控确认 regression diff 仅修改 `tests/g2.1-discovery-maps.test.ts`、`tests/fixtures/g2.1/inter-map-crash-worker.ts` 与本文；production、toolchain、`package-lock.json`、schema v1-v7、bundle v1-v6、既有 policy、Skill/agents/RFC bytes 全部冻结。`package-lock.json` SHA-256=`5271d765b9d1818ccace29aab770c629704a470c7c6a5827de07ddd4db68682e`，仓库只有一个 lockfile。
...
- 新增 immutable schema bundle `8.0.0`，保留 bundle `7.0.0` 与全部 v1-v8 schema bytes。v9 安装 `discovery_candidate.v1`、discovery `research_task.v2`、Evidence/Claim/Finding/Insight/Judgment/Source Manifest v2、`discovery_lane_result.v1`、`discovery_fan_in.v1`、`discovery_candidate_conversion.v1`、contract-only Envelope/Document Bundle 与 `discovery_candidate_policy.v1`。
- Candidate identity 只允许 `demand_seed | baseline_seed | solution_seed`，path 固定为 `artifacts/discovery/candidates/<candidate_id>.r<n>.json`。每个 rN 绑定 exact rN-1 ref/hash；Run、Scope、current Plan、profile、market、language、candidate kind、map ref/schema/id/revision/hash、fragment ref/JSON Pointer/id/status/hash 与 pre-thesis flags 不可漂移。Enrichment 只能产生新 revision、声明 exact changed fields，并对 typed Evidence chain/audit refs 执行 append-only 校验。
- Candidate producer/owner 固定 main Agent/G2.2；Research Task 由 main Agent 物化但 Harness 不 dispatch agent；lane-researcher 只拥有 task 指定的 Evidence chain 与唯一 lane result path；fan-in 由 main Agent拥有。Generation/evaluation 使用不同 task phase、Evidence role 与 Source Manifest group；overlap 必须 exact disclosure。
- Lane `disposition_id` 唯一，retained/watchlist/rejected sets 必须与 disposition records 精确相等且互斥，target 必须是 assigned typed candidate revision。Diversity/counterfactual retention 必须显式 retention basis。Fan-in reference-only，可显式把 lane candidate 升级到其 descendant revision；`failed | ignored_late | superseded` 不能进入 supporting refs 或 current enrichment basis，partial/insufficient 必须保留 conclusion ceiling。
- G2.3 conversion 只允许 `demand_seed -> demand_thesis.v1`、`baseline_seed -> baseline_option.v1`、`solution_seed -> solution_hypothesis.v1`，并绑定 retained/current source candidate、exact revision/hash/kind 和 fan-in。当前固定 `promotion_authorized=false`、`target_published=false`；旧 candidate immutable，conversion 不是 Evidence、外部验证或 validation success。
- v9 仅接入 deterministic validation surface。既有 `research-publication.v4` 与 Store adapters 保持 v1-v8/manifest bundle `7.0.0`；没有 v9 receipt/publication/recovery adapter，`artifact_envelope.v9` 在写入前以 `artifact.envelope_unsupported` fail closed。G2.2 Manifest adapter 也未安装，contract validation 不执行 state transition。

### Fixture 与验证证据

正 fixture 只含明确标记的 `SYNTHETIC ... contract fixture only` 数据，不包含真实 Evidence、URL、用户 quote、市场数据或 validation success。它覆盖三种 candidate kind、exact map fragment、r1->r2 enrichment、两个独立 source phase、typed Evidence-to-Judgment chain、两个 completed lane、exclusive disposition、reference-only fan-in 与 non-executable conversion proposal。

Negative catalog 共 32 个 mutation，覆盖 missing/wrong map fragment/ref/hash/revision、cross-Run/Scope/profile/market/language、subject kind、parent/hash/append-only/changed-fields、Decision-backed user correction、untyped Evidence/Judgment、disposition identity/overlap/missing target、fan-in non-candidate/overlap、source absence/overlap/phase drift、conversion path/parent/hash/kind/current-revision drift和 old bundle fail-closed。专项另覆盖 `partial | insufficient_evidence` 可引用，以及 `failed | ignored_late | superseded` 影响 current candidate 时 fail closed。

首次完整回归发现 `planning-contracts.test.ts` 仍把已由 v9 安装的 `discovery_lane_result.v1` 当作 downstream-uninstalled schema；测试白名单按 G2.2 ownership 精确修正，G2.4 future schema 继续 fail closed。修正后所有最终验收入口重新从头运行并通过。

| 命令 | 方案 A correction candidate 结果 |
| --- | --- |
| Frozen doctor（开工前） | PASS；Node `24.18.0`、npm `11.16.0`、skeleton `g2.1`、197/197 checks |
| G2.2 contract 专项 | PASS；13/13 tests，32/32 blocker/closure mutations，0 failed/skipped/todo |
...
## G2.2 Discovery Lanes / Fan-in runtime implementation checkpoint

中控已独立接受 directed contract repair `76089efebb1b6714a1ce44603aefeacf5d5fa63a`，其 parent=`be0b901345925504042eed7be337e848fe3595e2`。本 slice 以 clean `main@76089ef` 开工，按 `HYBRID_GATE_CONTINUOUS_V1` 在同一 worker task 内施工；未创建 subagent、worktree、Handoff 或重复 task，未 push、amend、rebase 或 reset。

### Runtime 交付

- 新增 schema bundle `9.0.0`、v10 Envelope/Document Bundle、`discovery_fan_in.v2` 与 `research-publication.v5`。已接受 v9/bundle `8.0.0` bytes 保持 immutable，v9 Store publication 继续以 `artifact.envelope_unsupported` fail closed；v10 使用 receipt v8 和 Manifest bundle `9.0.0`。
- Generic Artifact/Run Store 现在只接收调用方显式提供的 pre-thesis candidate、`research_task.v2`、typed Evidence/Claim/Finding/Insight/Judgment/Source Manifest、lane result 与 reference-only fan-in。Harness 不 dispatch agent、不执行 lane/pre-kill、不调用 LLM、不访问网络或执行 external validation。
- Research Task publication 只允许 pending-to-active 或 exact replay。Lane output 通过 immutable task 的 exact `allowed_output_path` 分类；`completed|partial|insufficient_evidence` 投影到 `completed_units`，`failed` 到 `failed_units`，已有 late/superseded unit 的 Artifact 只进入 `ignored_late_artifact_refs`。非法 transition 在 receipt/artifact/Manifest 写入前以 `artifact.discovery_lane_transition_invalid` 失败。
- v10 checkpoint/reopen/recovery 已安装。相同时间戳的 recovery transition 稳定按 Research Task -> ordinary Artifact -> terminal lane result 重放；temp-write 与 post-publish fault 可从 v8 receipt 确定性恢复。G2.3/G2.4 artifact types 仍由 v10 adapter blocked。
- Runtime fixture 使用真实 EvidenceStore v2 records 和 raw bytes 构造 substrate binding，不依赖固定假 hash。专项覆盖 staged publication、per-candidate pre-kill Judgment、fan-in closure、exact replay、partial/failed/late/superseded、零写入非法 transition、checkpoint/reopen、两类 fault recovery、v9 boundary 与 generic Harness/Skill publication。

### 验证与边界

全部命令使用 Node.js `24.18.0` / npm `11.16.0`：

| 命令 | G2.2 runtime checkpoint 结果 |
| --- | --- |
| `npm run test:g2.2` | PASS；10/10 tests（含 4 个 terminal-state subtests），0 failed/skipped/todo |
| `npm test` | PASS；230/230 tests，0 failed/skipped/todo |
| `npm run validate:fixtures` | PASS；122/122 tests，0 failed/skipped/todo |
| `npm run validate:schemas` | PASS；bundle `9.0.0`，95 schemas / 89 document validators，0 errors |
| `npm run typecheck` | PASS；`tsc --noEmit`，0 errors |
| `npm run lint` | PASS；Biome checked 244 files，0 errors |
| `npm run verify:skeleton` / doctor | PASS；skeleton `g2.2`，227/227 checks，Node `24.18.0` |
| `npm run test:g2.1` / `npm run test:recovery` | PASS；G2.1 34/34；recovery 11/11 |
| frozen bytes / forbidden-scope / `git diff --check` | PASS；v1-v9 schemas、bundle v2-v8、accepted policies 与 frozen toolchain files 零差异；无 runtime/orchestration/external-validation 越界；whitespace clean |

本 slice 不生成或声称真实 Evidence、candidate viability、Demand/Baseline/Solution/Opportunity Thesis、promotion、comparison、portfolio、市场验证或 validation success。G2.2 只标记 `IMPLEMENTED_PENDING_WHOLE_GATE_REGRESSION`，不逐 slice 请求独立验收；同一 task 继续 G2.3。

## G2.3 Demand / Solution / Thesis Synthesis runtime implementation checkpoint

本 slice 以 clean `main@3d9b720d938184517d089ecf5dd6235526f24f50` 开工，其 sole parent=`76089efebb1b6714a1ce44603aefeacf5d5fa63a`。按 `PERSISTENT_MODE_POLICY_V1` 复用同一 G2 worker/task，未创建 subagent、worktree、Handoff、逐 slice acceptance/regression 或独立账本提交，未 push、amend、rebase 或 reset。

### Runtime 与 contract 交付

- 新增 immutable schema bundle `10.0.0`、v11 Envelope/Document Bundle、receipt v9、`discovery-synthesis.v1` 与 `research-publication.v6`。accepted v1-v10 schemas、bundle v1-v9、既有 policies 和 conversion v1 bytes 保持 immutable；v10 继续 block executable conversion v2 与全部 formal G2.3 targets。
- G2.3 只执行 caller-supplied `discovery_candidate_conversion.v2`、Demand Thesis、Baseline Option、Solution Hypothesis、Solution Evaluation、Opportunity Thesis、Thesis Evaluation Snapshot 和 Merge 的 closed validation/publication。Harness 不生成或改写研究语义，不 dispatch agent、不调用 LLM、不访问网络或执行 external validation。
- Executable conversion 双向绑定 fan-in retained/current source candidate 与 formal target exact kind/revision/ref/hash。Demand 保持 solution-neutral；Baseline/Solution 必须回连其 typed candidate subject ancestry；旧 pre-thesis candidate 永久 immutable，conversion 不构成 Evidence、promotion success 或市场验证。
- 每个 formal thesis/evaluation 使用的 Claim/Insight/Judgment/Source Manifest/lane material 必须经 owning `research_task.v2` 绑定 exact source candidate ancestor。Generation/evaluation Source Manifest 保持 typed phase role、candidate partition 与 canonical source-group overlap disclosure；cross-candidate material 以 `synthesis.material_candidate_binding_mismatch` fail closed。
- Solution Evaluation 对同 Demand/Baseline 的每个 solution exact-once classified 并逐项比较 baseline。Opportunity exact 反映 selected/alternative solution 与 evaluation；Snapshot 在 enrichment 前冻结全部 thesis/dependency/source-group closure；Merge 对 frozen thesis exact-once 分类并强制 non-title semantic basis 与 diversity preservation。
- v11 bundle 使用稳定的 Demand conversion+target -> Baseline conversion+target -> Solution conversion+target -> evaluation -> Opportunity -> snapshot -> merge publication order。Store exact replay、Manifest bundle `10.0.0`、checkpoint/reopen 和 post-publish receipt recovery 已闭合；v11 继续 block G2.4 enrichment/Business Engine/comparison/portfolio/report types。
- RFC Artifact catalog/evaluator authority、Skill/reference、developer help/README 和 policy/schema docs 已同步。本文同一正常实现提交写入永久 `PERSISTENT_MODE_POLICY_V1`，明确 G2/G3/G4 每 Gate 一个连续 worker、Gate 末唯一一次 independent whole-gate regression，并取消逐 slice acceptance/regression/bookkeeping-only 流程。

### 验证、冻结与边界

全部命令使用 Node.js `24.18.0` / npm `11.16.0`：

| 命令 | G2.3 runtime checkpoint 结果 |
| --- | --- |
| `npm run test:g2.3` | PASS；6/6 tests、10 个 closed lineage/source/freeze/merge/order mutations，0 failed/skipped/todo |
| `npm run test:g2.2` / `npm run test:g2.1` | PASS；G2.2 10/10（含 4 terminal-state subtests）；G2.1 34/34；0 failed/skipped/todo |
| `npm test` | PASS；236/236 tests，0 failed/skipped/todo |
| `npm run validate:fixtures` | PASS；138/138 tests；入口已包含 G2.2/G2.3 fixtures；0 failed/skipped/todo |
| `npm run validate:schemas` | PASS；bundle `10.0.0`，108 schemas / 101 document validators，0 errors |
| `npm run lint` / `npm run typecheck` | PASS；Biome checked 264 files，0 errors；`tsc --noEmit` 0 errors |
| `npm run verify:skeleton -- --json` / doctor | PASS；skeleton `g2.3`，248/248 checks，Node `24.18.0` |
| frozen bytes / forbidden-scope / `git diff --check` | PASS；相对 `3d9b720` 的 119 个既有 schema/policy JSON paths 零差异，tree digest=`d2f5d8a6480eb53ecb7cb20f957fc12926209b08e1217534e4cb280b4502e065`；`package-lock.json`、`.node-version`、`.npmrc`、`tsconfig.json` 零差异；变更生产源码无 network/LLM/process-dispatch surface；whitespace clean |
...
## G2.4 Enrichment / Business Engine / Comparison rejected implementation exit candidate
...
该 candidate 最终 commit=`3dbe755db2dd88603a15e20c1c30c3c369ab7651`，sole parent=`a8a42b9b29b4b5ca317ab8a346b39a5d4c39bfef`。fresh independent boundary acceptance task `019fa7fa-6dbd-78d0-a5df-67d7ac9a1eec` 后续判定 `FAIL`；因此本节的历史 PASS 只代表当时入口结果，不能继续作为 G2 boundary acceptance 或开放 G3 的依据。五项 finding 与定向修复记录见下一节。

## G2 fresh boundary acceptance directed repair candidate

本返修以 rejected candidate `3dbe755db2dd88603a15e20c1c30c3c369ab7651` 为 exact parent；`fixed_ruleset_id=startup-opportunity-controller-fixed-v2`。fresh independent acceptance task `019fa7fa-6dbd-78d0-a5df-67d7ac9a1eec` 的结论为 `FAIL`，普通质量入口的历史 PASS 不能覆盖其五项 semantic bypass。本返修继续使用原 G2 Construction Stage 主 task，未创建替代 worker、worktree、Handoff 或下游 task，未 push、amend、rebase 或 reset；没有启用只读 subagent。

### Versioned authority 与 deterministic repair

- 新增 immutable schema bundle `12.0.0`、v13 Envelope/Document Bundle、receipt v11、`research-publication.v8`、`discovery-evaluation.v2`、`discovery-adaptation-binding.v1` 与 `report_consistency_evaluation.v3`。既有 v1-v12 schema、bundle v1-v11 与 published policy bytes 不改写，旧 Store adapter 条目语义不变；v13 继续 block 全部 G3 AI Artifact。
- Validator 现在从每个 Opportunity 的 `selected_solution_ref` 解析 exact Solution Hypothesis `uses_ai`。G2 不生成 G3 bundle；因此 `uses_ai=true` 且缺 G3 bundle 时，`ai_mandatory_bundle` 只能为 `insufficient_evidence`，comparison/recommendation conclusion ceiling 至多 `investigate_further`。AI/hybrid full-chain fixture 覆盖 `passed|not_applicable` 和 selected-solution toggle fail-open。
- Decision tier 使用 closed tier order 并绑定 exact portfolio first bet、该 opportunity comparison、hard gates、fan-in ceiling 和四个 panel sufficiency；null first bet、insufficient first bet 或 mixed not-ready inputs 不得发布 `prioritize`。
- Deterministic forbidden-expression scan v1 同时扫描 structured `report.v1`、`decision-brief.md` 与 `report.md`；validation-success、probability 和 global-score policy 命中产生稳定 hash-bound match 并 fail closed。`build-report` 在任何 write 前返回 `report.forbidden_expression_detected`；generic publication、checkpoint、crash/reopen 与 recovery 均重新验证相同 scan contract，不再由 caller narrative 获得固定 `passed`。
- `candidate_pre_killed -> skip_unit` 通过 versioned adaptation binding policy 绑定 Gap `subject_ref` 与 pending/enabled unit 的 exact candidate revision input。只有只服务该被淘汰 candidate 的 unit 可以 skip；missing target 或同时服务 retained/other candidate 的共享 unit 分别以 `adaptation.pre_kill_candidate_target_mismatch`、`adaptation.pre_kill_shared_candidate_skip_forbidden` 在 Plan write 前失败，共享 unit 必须保留或 supersede。
- Whole-G2 fixture 现在覆盖 `general|industry_first|ai_first|hybrid` 四条 candidate-to-report chain，保留非空 counterfactual refs、semantic merge diversity 与 AI ceiling。所有内容继续显式标记 synthetic/unverified，不构成 Evidence、市场事实、用户 quote 或 validation success。

### 验证、冻结与风险

全部命令使用 Node.js `24.18.0` / npm `11.16.0`：

| 命令 | Acceptance-directed repair candidate 结果 |
| --- | --- |
| G2.1 / G2.2 / G2.3 / G2.4 专项 | PASS；34/34、10/10、6/6、25/25 tests；G2.4 含 11 个新增 semantic cases、4 profile chains 和 stable negative codes |
| G0.4 adaptation / Store faults / Store recovery | PASS；36/36、10/10、11/11 tests；覆盖 pre-kill apply/replay/crash/reopen 与既有 receipt recovery |
| `npm test` | PASS；265/265 tests，0 failed/skipped/todo |
| `npm run validate:fixtures` | PASS；163/163 tests，0 failed/skipped/todo |
...
| `npm run verify:skeleton -- --json` / frozen doctor | PASS；skeleton `g2.4`，296/296 checks，Node `24.18.0` |
| frozen bytes / forbidden-scope / `git diff --check` | PASS；相对 `3dbe755` 的 165 个既有 schema/policy JSON paths 零差异，base blob/path manifest digest=`5b355f16d548527ebb7190e1114aa210e9f53a56b0786d36fc05f687c89a73e7`；`package.json`、`package-lock.json`、`.node-version`、`.npmrc`、`tsconfig.json` 零差异；无 network/LLM/agent dispatch/external-validation/G3 runtime/DAG runtime 越界；whitespace clean |
...
该历史 second repair candidate 最终 commit=`e68be2dc17ba2e7e29be64dc1620fe5541ad6ad0`。replacement fresh independent boundary acceptance task `019fac4a-24f1-7163-b985-84bfac2cb417` 后续统一判定 `FAIL`：v3 receipt 的全局 boolean bypass 跳过了 G2.1/G2.2 domain validation，且新增 synthesis semantic check 未按 v12/v13 分流。本节 PASS 只能作为 rejected candidate 的历史入口证据；其中“豁免 historical domain pass”的实现结论已被证明错误，不得继续作为 current recovery contract。

## G2 replacement acceptance third directed repair candidate

本返修以 rejected candidate `e68be2dc17ba2e7e29be64dc1620fe5541ad6ad0` 为 exact sole parent，`fixed_ruleset_id=startup-opportunity-controller-fixed-v2`。replacement fresh independent acceptance task `019fac4a-24f1-7163-b985-84bfac2cb417` 的结论为 `FAIL`；原 G2 Construction Stage 主 task 继续作为唯一 tracked-files、Git index、账本与 commit 写入者。未创建替代 worker、subagent、worktree、Handoff、automation 或下游 task，未 push、amend、rebase 或 reset。

### 两项定向闭合

- 移除 `validateHistoricalDiscoveryContracts` 全局 boolean bypass。已验证 `plan_revision_operation.v3` receipt 现在向 Artifact validator 传递 exact historical Plan ref/hash/revision 与 bound candidate refs；G2.1/G2.2 各自只在该 binding 与 durable Plan/candidate/map lineage 闭合时构造只读 historical Manifest/Plan view，并始终执行完整 domain validator。合法 r1/r2 双 Plan apply、pending replay 与 reopen 保持通过；map profile 漂移或未被 receipt 绑定的 solution candidate profile 漂移即使同步重算 Envelope hash、Artifact operation key、receipt filename/content 和 durable bytes，也分别在 replay 以 `artifact.reference_invalid`、在 reopen 以 `recovery.reference_invalid` fail closed。
- `synthesis.solution_candidate_semantic_drift` 只对 `document_bundle.v13` 生效；v11/v12 保留 parent validator behavior。该 candidate 当时把 size=`285141`、SHA-256=`839afaea562249c395a8a1e052d54563eb6ce8e9b4ea1573e815148767b37f44` 记录为 parent-produced v12 fixture；第四次 acceptance 后已确定性证伪：它不是 exact parent builder bytes，在 `e68be2d` validation 以 `synthesis.solution_candidate_semantic_drift` 失败，其 publication、v10 receipt、Manifest、checkpoint、recovery、reopen 均为 `NOT_RUN`。本段不得继续作为兼容性证据；current replacement 与双运行时结果见后续 fixed-v3 audit 节。

### 验证、冻结与风险

全部命令使用 Node.js `24.18.0` / npm `11.16.0`：

| 命令 | Third acceptance-directed repair candidate 结果 |
| --- | --- |
| `npm run test:g0.4` | PASS；56/56 tests；新增 5 个 historical Plan cases，覆盖合法双 Plan、G2.1/G2.2 receipt-resigned drift 的 reopen/pending replay；0 failed/skipped/todo |
| G2.1 / G2.2 / G2.3 / G2.4 专项 | 当时 PASS；34/34、10/10、6/6、29/29 tests，0 failed/skipped/todo；但 G2.4 的 `839...` fixture provenance 和所谓 parent lifecycle 后续被证伪，不能作为 current acceptance evidence |
| `npm run test:g1.3` / `test:g1.4` / `validate:store` | PASS；25/25、36/36、11/11 tests；0 failed/skipped/todo |
| `npm run test:faults` / `npm run test:recovery` | PASS；10/10、11/11 tests；0 failed/skipped/todo |
| `npm test` | PASS；289/289 tests，0 failed/skipped/todo |
| `npm run validate:fixtures` | PASS；167/167 tests，0 failed/skipped/todo |
| `npm run validate:schemas` | PASS；bundle `12.0.0`，141 schemas / 133 document validators，0 errors |
| `npm run lint` / `npm run typecheck` | PASS；Biome checked 310 files，0 fixes/errors；`tsc --noEmit` 0 errors |
| `npm run verify:skeleton` / frozen doctor | PASS；repository doctor 296/296，Node `24.18.0` |
| frozen bytes / forbidden-scope / `git diff --check` | PASS；相对 `e68be2d` 的 175 个 schema/policy JSON paths 零差异，base authority tree digest=`8978917f632439e128f2b529fd7100a6f72f7af4fb1bc260961453c1485780d5`；`package.json`、`package-lock.json`、`.node-version`、`.npmrc`、`tsconfig.json` 零差异；无 G3/Skill/agent/runs、network/LLM/external-validation/Workflow Runtime/DAG/daemon/UI/DB 越界；whitespace clean |

残余风险保持显式：historical Plan selection 是 validated v3 receipt 的内部 recovery context，future receipt/version 若改变 Plan/candidate ownership 必须发布新 migration contract，不能恢复 boolean skip；固定 v12 fixture 保证已知 parent bytes/behavior，不能替代对所有未来历史 Run 的无限兼容证明。G3 mandatory bundle 仍未实现，任何 AI/hybrid 结论继续受 insufficient-evidence ceiling。

本返修不修改或新增任何 schema/policy JSON authority，不改变 package/toolchain bytes，也不实现 G3、external validation、network research、agent/LLM dispatch、通用 Workflow Runtime、DAG DSL、daemon、UI 或 DB。所有 synthetic fixture 只证明 deterministic contract/runtime compatibility，不构成 Evidence、candidate viability、市场验证或 validation success。原子 repair commit 使用本节占位避免 self-reference，sole parent 必须为 `e68be2dc17ba2e7e29be64dc1620fe5541ad6ad0`；状态仍仅为 `G2_REPAIR_CANDIDATE_PENDING_FRESH_INDEPENDENT_REGRESSION`，G2.R、G3-G4 保持 `NOT_READY`。

该历史 third repair candidate 最终 commit=`230064e707622c9296dec3bbecb725d9f7b6b691`。fresh independent acceptance `019facbe-2b39-71e2-9ef7-3d9576717f18` 后续统一判定 `FAIL`：三个正式 report surface 均漏扫 `score global|score-global|score_global`，且 `839...` fixture provenance 及其 parent lifecycle 声明可确定性证伪。至此 G2 连续有效 boundary FAIL 累计为 4：`019fa7fa...`、`019fa881...`、`019fac4a...`、`019facbe...`；`019fac2b...` 仅 systemError、无统一结论，不计数。fixed-v3 强制进入 `NON_CONVERGENT_AUDIT`，不得继续点状返修或立即创建下一次 acceptance。

## G2 fixed-v3 NON_CONVERGENT_AUDIT 与 systemic repair candidate

本次以 `230064e707622c9296dec3bbecb725d9f7b6b691` 为 exact sole parent，`fixed_ruleset_id=startup-opportunity-controller-fixed-v3`。原 G2 Construction Stage 主 task 继续是唯一 tracked-files、Git index、账本与 commit 写入者；两个只读 subagent `audit_history_matrix`、`audit_report_oracles` 只做历史矩阵、regex/real-entry、fixture/provenance 和 cross-version oracle 审查，未修改任何文件、Git、账本或 automation，未 commit 或创建 descendant。主 worker在审计结束并明确 `NOT_BLOCKED_BY_SPEC` 前未编辑生产代码。

### 四次 FAIL 汇总与根因分类

- 第一次 `019fa7fa...` / `3dbe755`：AI mandatory gate、Recommendation ceiling、固定 report consistency `passed`、shared pre-kill 和缺失四 profile/diversity chain 五项；AI、基础 ceiling、shared binding、profile/diversity 已修复，report finding 后续以新 property class 复发。
- 第二次 `019fa881...` / `3a5e51d`：typed candidate binding、lower fan-in ceiling、三个 report boundary、v12/v13 evaluation dispatch 和伪 profile chain 五项；typed binding、strictest ceiling、profile chain 已修复，v12 dispatch 与 report finding分别进入后续 cross-version/property 审计。
- 第三次 `019fac4a...` / `e68be2d`：v3 receipt 全局跳过 G2.1/G2.2 domain validation、synthesis semantic check 倒灌 v12 两项；exact historical Plan view 与 v13-only synthesis 已修复并被第四轮确认，所谓 parent fixture 证据后续被证伪。
- 第四次 `019facbe...` / `230064e`：reverse-order global score 三 surface fail-open、`839...` 非 exact parent builder bytes 两项；均为本次 systemic repair 的 open 输入。
- 验收基线遗漏：上位 baseline 已明确 real entry、正反 token order、三 separator、三 surface、pre-write zero-write、exact parent bytes 与 parent runtime；施工 preflight 没有把它机械化为完整 property/mutation matrix，普通 suite 的 PASS 因此不能覆盖遗漏。
- 共同架构根因：lexical policy 由已知例句 regex 手工枚举；单 Artifact、bundle、Store、ReportRuntime、recovery 的 version/lifecycle dispatch 分散；compatibility test 只调用 current runtime。历史 boolean bypass、v12 semantic drift 与本次 report scan version drift 都属于这个族，而不是独立 typo。
- fixture/oracle 失真：`839afaea...` / 285141 bytes 内 source candidate `uses_ai=false`、formal Solution `uses_ai=true`，在 producer parent `e68be2d` validation 必然报 `synthesis.solution_candidate_semantic_drift`；其 publication/v10 receipt/Manifest/checkpoint/recovery/reopen 全部 `NOT_RUN`。G0.4 “parent receipt v1”和 G2.3 “frozen synthesis index”等 current-runtime compatibility tests 也不得升级表述为 parent-produced byte oracle。
- 规范歧义：无。RFC 2356、3948、3991、3995、4011、4229 与已发布 v1/v2 policy 已唯一规定 exact historical Plan validation、v13-only repaired semantics、三个 formal surface、反向语序和 pre-write zero-write；无需新 schema/policy 或业务语义选择，结论为 `NOT_BLOCKED_BY_SPEC`。

### Invariant-to-test matrix

| ID | 上位 authority 与 invariant | 真实 entry / Artifact / lifecycle | mutation/property 与历史 finding | 既有 coverage / blind spot | fixture / provenance oracle | expected fail-closed / zero-write | repair owner / shadow evidence |
| --- | --- | --- | --- | --- | --- | --- | --- |
| H1 | RFC 3948/3995；evaluation policy v2：selected Solution `uses_ai` 决定 missing-G3 ceiling | Bundle validator；Opportunity -> Solution -> hard gate -> Comparison/Report | AI/hybrid `uses_ai=true` × passed/not_applicable；`019fa7fa`/`3dbe755` | 已覆盖三种 selected-solution mutation；无 current blind spot | 四 profile exact map/candidate/conversion/formal Solution refs+hashes | `g2_4.ai_mandatory_bundle_gate_violation`；publication zero-write | evaluation validator；G2.4 profile/AI table与 shadow `32/32` PASS |
| H2 | RFC 3948/3995；decision tier 取 strictest closed ceiling | Fan-in -> Comparison/panels/gates -> Portfolio/first bet -> Recommendation | insufficient/mixed/null first bet；`019fa7fa`/`3dbe755` | 后续 H7 暴露 lower fan-in遗漏；现已闭合 all tiers | exact fan-in/comparison/portfolio refs+hashes | `g2_4.decision_tier_ceiling_violation`；report不可发布 | evaluation validator；两组 table-driven ceiling tests PASS |
| H3 | RFC 4011/4229；Consistency v3 不能由 caller 固定 `passed` | structured report + brief + full report；ReportRuntime/build/checkpoint/reopen | validation/probability/global-score；`019fa7fa`/`3dbe755` | 原 runtime 固定 `[]/passed`；修后仍有 H8/H13 property blind spot | v13 explicit report envelope与 derived sidecars | validator `g2_4.forbidden_report_expression`；Store `artifact.schema_invalid`；runtime `report.forbidden_expression_detected`；4 formal/3 materialized/receipts/Manifest zero-write | report scanner/runtime/Artifact validator；完整 tree snapshot与 7 fault boundaries PASS |
| H4 | RFC 2356/4007；pre-kill 只能 skip exact exclusive pending candidate unit | adaptation validate/apply/v3 receipt/replay/crash/reopen | shared candidate unit；`019fa7fa`/`3dbe755` | H6 后续暴露 path-only；当前 shared/missing均覆盖 | exact candidate Envelope+Plan+unit inputs | `adaptation.pre_kill_shared_candidate_skip_forbidden`；Plan/receipt/Manifest zero-write | adaptation/plan runtime；G0.4 `56/56` PASS |
| H5 | RFC 3948；profile/diversity 必须贯穿 G2.2+ | candidate -> lane/fan-in -> merge -> report | 仅 general、counterfactual empty；`019fa7fa`/`3dbe755` | H10 后续暴露 G2.3 直接翻 AI；当前逐阶段闭合 | general/industry/AI/hybrid owned map fragments | profile/diversity lineage drift fail closed；无错误 profile publication | fixture builders + map/candidate/synthesis/evaluation validators；四 profile test PASS |
| H6 | RFC 2356/4007；pre-kill subject 是 exact typed candidate revision | validate/apply、parent-produced v1/v2与 v3 receipt、replay/recovery | missing/null/non-envelope/wrong type/Run/Plan/hash/revision；`019fa881...`/`3a5e51d` | 当前 malformed matrix与 durable drift闭合 | durable candidate document+Envelope hash、base Plan ref/hash/revision | `adaptation.pre_kill_candidate_binding_invalid`；任何 receipt/Artifact/Manifest write 前失败 | adaptation/plan runtime；G0.4 malformed/replay matrix PASS |
| H7 | RFC 3948/3995；lower fan-in ceiling也进入 strictest Recommendation | fan-in reject -> comparison insufficient -> recommendation | five fan-in tiers × comparison/gate/panel/portfolio；`019fa881...`/`3a5e51d` | 原 H2 只测 comparison；当前全 tier table闭合 | fan-in conclusion ceiling exact ref/hash | `g2_4.decision_tier_ceiling_violation`；无 Recommendation/report write | evaluation validator；strictest tier table PASS |
| H8 | RFC 4011/4229；三个 surface 是独立 pre-write boundary | generic Store direct structured/brief/view；checkpoint/recovery/reopen | hyphen/underscore/语序 variant；`019fa881...`/`3a5e51d` | 原只测一个复合正向句与部分文件不存在；未比较 whole tree | derived v13 sidecars重算 markdown/content hashes | 同 H3 stable codes；Artifact receipt、materialization receipt/file、Manifest完整 tree byte-identical | Artifact validator + report runtime；三 surface × 三 rule reverse cases PASS |
| H9 | RFC 3948/3995；v12 使用 frozen evaluation v1，v13 才使用 v2 | bundle validation/publication/v10 receipt/Manifest11/checkpoint/reopen | same AI gate mutation across v12/v13；`019fa881...`/`3a5e51d` | evaluator dispatch已闭合；本次补 report scan同一集中 version predicate | document bundle/envelope version + published policy adapter | v12保持历史结果；v13 AI/report mutation fail closed | Artifact/evaluation/report dispatch；v12/v13 same-mutation test PASS |
| H10 | RFC 3948；AI semantics 必须从 AI map fragment进入 typed candidate | G2.1 map -> G2.2 candidate -> G2.3 conversion/Solution -> Opportunity -> G2.4 report | G2.2 ordinary/false、G2.3直接翻 AI；`019fa881...`/`3a5e51d` | 当前四 profile逐阶段 refs/hash/semantic差异闭合 | profile-owned exact map fragment/ref/hash/revision | candidate/synthesis semantic drift stable code；无伪 profile report | fixture builders + synthesis validator；四 profile chain PASS |
| H11 | RFC 2356；receipt只授权 exact historical Plan view，domain validators永远执行 | completed `RunStore.load`、pending `after_intent` replay | re-signed G2.1 map drift、unbound G2.2 candidate drift；`019fac4a...`/`e68be2d` | 当前无 global boolean bypass；第四轮明确重放通过 | v3 receipt exact Plan ref/hash/revision/bound candidates | replay `artifact.reference_invalid`；reopen `recovery.reference_invalid`；无 recovery write | Artifact validator/RunStore；G0.4 historical matrix PASS |
| H12 | RFC 3991；Solution semantic preservation只属于 v13 repair | synthesis bundle validate + v12 publication lifecycle | source/formal `uses_ai` drift across v11/v12/v13；`019fac4a...`/`e68be2d` | version dispatch已修；旧 `839...` oracle失真由 H14替换 | identical mutation + bundle version；exact a08 clean bytes | v11/v12 frozen behavior；v13 `synthesis.solution_candidate_semantic_drift` | synthesis/Artifact validator；v13 drift + a08 dual-runtime PASS |
| H13 | RFC 4011/4229 与开工 baseline；三 rule正反 order/三 separator/三 surface | pure scanner、direct Artifact/Store、ReportRuntime 7 faults、checkpoint/load | `score global|score-global|score_global`；`019facbe...`/`230064e` | 原 scanner只 forward examples；现为 rule × order × separator × surface property cross | normalized text stable hash `<rule>@<surface>:<sha256>`；v13 explicit Artifacts | H3 stable codes；每次拒绝前后完整 Run tree byte-identical | report-consistency + centralized v13 predicate；repo内/仓库外 G2.4 `32/32` PASS |
| H14 | immutable compatibility：parent-produced必须闭合 producer commit/invocation/bytes/runtime | exact builder -> validation -> staged publication -> v10 receipt -> Manifest11 -> fault recovery -> checkpoint/reopen | `839...` false provenance；`019facbe...`/`230064e` | 旧 test仅 current runtime且 parent lifecycle `NOT_RUN`；已删除 self-oracle | `e68be2d` `setup(...,"cross-version-v12","ai_first") -> legacyV12` exact `a08eb3...` / 285157 bytes | parent/current均 valid；34 个 v12 receipts全为 v10；Manifest `11.0.0`；recovery/reopen PASS；相同 final tree digest | `git archive` parent executor + current executor；tree 181 files、digest `sha256:07fcfd7a5982fdee09ca899b95eecc8cae47177f6d9ed95a99ba99d8563d3dd6` |

### Systemic repair 与 shadow evidence

- `report-consistency.ts` 保留 NFKC 与 `_/-/space` 统一 normalization，但三条 rule 都按双向 token order 关闭 property class：market validation success、probability/success/chance、global/overall score。相同 tokens 的三 separator 必须产生同一个 normalized match hash；非禁止的 pending/local wording仍保持不命中。
- 单一 `requiresDeterministicReportScan` predicate 同时约束单 Artifact validator 与 ReportRuntime；只有 envelope v13 执行 Consistency v3 scan，v12继续生成冻结 Consistency v2 `passed/[]`，不把 repaired semantics倒灌历史版本。
- committed fixture替换为 exact parent builder output `a08eb3cc38ba099a3fd912cb90d49fe9039507e82fb4eb0c701da08e0176dee6` / 285157 bytes。cross-version driver在无 worktree 的 `e68be2d` `git archive` 与当前源码树中分别重建 exact bytes并执行完整 lifecycle；两侧输出及最终 tree digest exact 相同。
- formal boundary tests对每条 rule使用此前漏过的 reverse/separator形式，覆盖 structured report、独立 decision brief、独立 full report、generic Store `after_intent`、ReportRuntime全部 7 fault boundaries、checkpoint后 load/reopen；每次拒绝前后比较完整 base64 Run tree，确认 4 formal Artifacts、3 materialized outputs、receipts和Manifest均零新增。
- repository-external shadow candidate从 `230064e` archive起步，只覆盖本次 candidate diff，并显式使用原 Git object database重放 parent executor；G2.4 `32/32` PASS，0 failed/skipped/todo。该 shadow是 candidate preflight，不是 independent boundary acceptance，也不授权 G3。

全部命令使用 Node.js `24.18.0` / npm `11.16.0`：

| 命令 | fixed-v3 systemic repair 结果 |
| --- | --- |
| `npm run test:g0.4` | PASS；56/56，historical exact Plan/domain、typed pre-kill、apply/replay/crash/reopen；0 failed/skipped/todo |
| G2.1 / G2.2 / G2.3 / G2.4 专项 | PASS；34/34、10/10、6/6、32/32；0 failed/skipped/todo |
| `npm run test:g1.3` / `test:g1.4` / `validate:store` | PASS；25/25、36/36、11/11；0 failed/skipped/todo |
| `npm run test:faults` / `npm run test:recovery` | PASS；10/10、11/11；0 failed/skipped/todo |
| repository-external shadow G2.4 | PASS；32/32；parent/current exact a08 builder+lifecycle与 full candidate preflight；0 failed/skipped/todo |
| `npm test` | PASS；292/292，0 failed/skipped/todo |
| `npm run validate:fixtures` | PASS；170/170，0 failed/skipped/todo |
| `npm run validate:schemas` | PASS；bundle `12.0.0`，141 schemas / 133 document validators，0 errors |
| `npm run lint` / `npm run typecheck` | PASS；Biome checked 311 files，0 fixes/errors；`tsc --noEmit` 0 errors |
| `npm run verify:skeleton` / frozen doctor | PASS；repository doctor 296/296，Node `24.18.0` |
| frozen bytes / toolchain / forbidden scope / `git diff --check` | PASS；相对 `230064e` 的 175 个既有 schema/policy JSON paths 零差异，content+path manifest digest=`d8aa725756c996f2c9474cd88a34dbd66f5c6520129a1b902d47d4a9153e9a91`；`package.json`、`package-lock.json`、`.node-version`、`.npmrc`、`tsconfig.json` 零差异；生产代码无 process/network/LLM/external-validation/G3/Workflow Runtime/DAG/daemon/UI/DB surface；whitespace clean |

本次没有修改 RFC 业务语义，也没有修改或新增 schema/policy JSON authority；现有 RFC 与 published authority 已唯一消解 findings。残余风险保持显式：deterministic lexical grammar只能执行已发布 rule，不替代 Evidence truth/quality audit；`git archive` differential oracle要求 producer commit object仍可用；未来若改变 report grammar、receipt/Plan ownership或历史版本行为，必须发布新 versioned authority，不能扩张 v13 predicate或恢复 boolean bypass。
...
## G3 fixed-v5 Construction Stage

当前 G3 Stage 以 accepted G2 candidate `6d09cc7afbe2a3638f627f5e2fe4fe90412c683c` 为 exact sole parent，`fixed_ruleset_id=startup-opportunity-controller-fixed-v5`。fresh independent whole-boundary acceptance task `019fada4-6e70-7ce1-8b0e-2756ef0a9f20` 已对该 candidate 给出统一 `PASS`，H1-H14闭合、required `NOT_RUN=0`、required `SECURITY_VALIDATION_NOT_RUN=0`；因此 G2=`DONE`，历史连续 FAIL count 在 Stage 切换时清零，G3成为唯一 active Stage，G4=`NOT_READY`。本 Stage 不创建 subagent、worktree、Handoff或下游 task；同一主 worker是 tracked files、Git index、账本与 commit 的唯一写入者。

### G3.1 AI baseline / reliability / data contracts

- 新增 composed schema bundle manifest v3：bundle `13.0.0` 显式继承 immutable bundle `12.0.0`，只追加 v14 schema。loader对 local base path、cycle、duplicate id/file/document version、完整 `$ref` 做确定性检查；不修改任何既有 schema/policy JSON bytes。
- 新增 caller-supplied `capability_evidence.v1`、`ai_capability_benchmark.v1`、`ai_evaluation_reliability.v1` 与 `ai_data_dependency.v1`。合同固定 generic model、platform-native、open-source 三类 baseline，代表性/limited/desk-research-only声明，quality/latency/failure/human boundary，以及 data rights、ground truth、feedback、provider portability、privacy/deletion/export边界。
- `AiBundleValidator` 验证 exact Opportunity subject -> selected Solution `uses_ai=true` -> versioned trigger lineage、每个 required dimension恰好一个 result、specialized Artifact type与同 lineage、reliability exact benchmark binding、freshness window。Harness不运行 benchmark/research、不生成 baseline/Judgment或 Evidence、不调用 agent/LLM、不访问 network。
- publication policy v9以 canonical hash绑定 immutable v8 base并只追加 v14/document bundle v14/receipt v12 adapter；v13继续对 G3类型 fail closed，v14继续 block G3.2 economics/commoditization/trust与G3.3 mandatory bundle。ArtifactStore/RunStore已安装 v14/receipt v12 validation、publication rank、checkpoint/reopen/recovery dispatch。
- fixture全部标记 `SYNTHETIC` / `unverified`；representative benchmark只证明 deterministic mechanics，不构成真实 Evidence、外部 validation、成功 benchmark、产品可行或市场成功。

本 slice 专项与受影响验证均使用 Node.js `24.18.0` / npm `11.16.0`：`test:g3.1` PASS 7/7；`validate:fixtures` PASS 177/177；`validate:schemas` PASS（bundle `13.0.0`，149 schemas / 140 document validators）；`typecheck` 与 `lint` PASS；`test:g2.4` PASS 32/32；`validate:store` PASS 11/11；`test:faults` PASS 10/10；`test:recovery` PASS 11/11；schema-validation PASS 13/13；G1.1 PASS 11/11、G1.2 PASS 20/20、G2.1 PASS 34/34；上述完成项均为 0 failed/skipped/todo。首次并发 `validate:fixtures` 暴露旧 test对 default bundle `12.0.0`/141/133及12 adapters的 current-default断言，已随实现更新为 `13.0.0`/149/140及13 adapters；旧 planning validator只安装 bundle v2.1/v5，publication overlay loader现仅在该旧 bundle安装 base policy schema时做 schema校验，并始终核对 canonical base hash，从而保留旧 adapter contract且避免把 v14 schema倒灌历史 bundle。后续完整 `validate:fixtures` 已通过；完整 full quality入口仍在 G3 Stage candidate preflight重放。
...
G3.2 原子提交=`4d727dfa5fd60bb06c73ccdf553ef47acce9e1f8`，sole parent=`bed46a4a9f4d977155b68e4d5fbbed6929d9cf55`。G3.2状态仅为 `IMPLEMENTED_PENDING_WHOLE_GATE_REGRESSION`；同一 worker连续进入 G3.3，不逐 slice等待验收，不标 G3 `DONE`，不开放 G4。

### G3.3 mandatory bundle dynamic coverage

- 新增 composed bundle `15.0.0`，只在 immutable bundle `14.0.0` 上追加 v16 `ai_mandatory_bundle.v1`、closed consumer binding、evaluation policy v3、publication policy v11、Envelope与Document Bundle。v12固定dispatch evaluation policy v1，v13-v15固定dispatch v2，只有v16使用v3；历史schema/policy JSON bytes不改写。
- versioned trigger固定六维顺序：`capability_frontier`、`cost_and_deployment`、`workflow_and_human_boundary`、`ecosystem_and_platform`、`data_and_evaluation`、`adoption_and_trust`。caller必须显式提交每维 `covered | insufficient_evidence | not_applicable`；source unavailable只能是`insufficient_evidence`，不能伪装`not_applicable`。Harness只验证exact Opportunity -> selected Solution `uses_ai` -> trigger lineage、七类G3.1/G3.2输入/ref/hash、coverage计数、freshness/continuation聚合与ceiling，不生成coverage、Evidence、bundle或语义结论。
- v16在comparison A、Recommendation、Traceability、structured report以及Harness派生的Decision Brief/Report View/Consistency sidecar envelope上保存`ai_bundle_binding`。`bound`绑定exact bundle ref/hash/status/ceiling；`missing`显式保留null identity和降级ceiling；selected `uses_ai=false`只接受`not_required`。missing/incomplete/desk-research-only/stale coverage不得支持`prioritize`。
- publication policy v11以canonical hash绑定immutable v10 overlay，只追加v16/document bundle v16/receipt v14 adapter。ArtifactStore/RunStore安装v16 publication rank、checkpoint、reopen与recovery dispatch；report runtime只确定性继承caller binding，不合成AI判断。合法synthetic lifecycle覆盖publication -> report materialization -> checkpoint -> clean reopen，未构造异常receipt/Manifest current-state。
- fixture保持`SYNTHETIC`/`unverified`；代表性benchmark、0 volume/cost、desk-only bundle与non-AI binding只证明deterministic mechanics，不构成真实Evidence、成功benchmark、外部validation、产品可行或市场成功。

G3.3普通专项最终应记录fixed六维、summary、source-unavailable、missing ceiling、non-AI `not_required`、stale continuation、v12-v16 dispatch、derived report binding与合法publication/checkpoint/reopen；所有required普通测试必须0 failed/skipped/todo。Stage candidate full quality与精确计数记录在本节后续preflight段，不得用主worker自检声称boundary acceptance。

fixed-v5 security-sensitive mapping：bound consumer bundle hash、publication v11 predecessor hash、receipt v14 identity与Manifest current-state保护只记录静态源码`validateAiBundleContract`、`expandPolicy`、`validateArtifactReceipt`、`validateEnvelopeBoundary`、`RunStore.recoverLocked` invariant，并映射仓库既有store-faults/store-recovery测试；这些未执行的新异常分支为`SECURITY_VALIDATION_NOT_RUN(reason=fixed-v5 forbids new dynamic Artifact/receipt/Manifest identity, integrity, or current-state adversarial probes)`，不得记PASS。需要构造受保护authority state的数据权利/privacy/trust分支为`SECURITY_VALIDATION_NOT_RUN(reason=fixed-v5 permits closed-shape/static mapping only; no protected authority-state mutation was constructed)`。construction中一项早期草稿mandatory-input-hash mutation曾在分类纠正前本地执行；该测试已从candidate删除，结果不计入PASS或security coverage，该分支只记录为protocol deviation，既不记PASS也不伪称`SECURITY_VALIDATION_NOT_RUN`。

#### G3 future acceptance C1 baseline

| C1项 | future acceptance必须核对的冻结基线 |
| --- | --- |
| authority / lineage | `fixed_ruleset_id=startup-opportunity-controller-fixed-v7`、immutable block SHA-256=`2279bb7a315c39a51cc215f26c0fc4c2fb46b5756a7e42cee0f331c2fe8af1f5`；accepted G2=`6d09cc7afbe2a3638f627f5e2fe4fe90412c683c`；G3 lineage=`bed46a4` -> `4d727df` -> `b54fd76` -> `1e8969a` -> 本次clean sole-child repair candidate；不得amend/rebase/reset/push |
| real entry / Artifact / environment | Node `24.18.0`、npm `11.16.0`、TypeScript `7.0.2`、唯一package-lock v3；`validate-artifact`、ArtifactStore、RunStore、`build-report`、checkpoint/load/recovery真实入口；bundle `15.0.0`、envelope/document bundle v16、receipt v14、publication v11、evaluation v3、`ai_mandatory_bundle.v1`与v16 consumer binding |
| compatibility boundary | v1-v13既有adapter/receipt/schema/policy bytes与行为必须冻结；v14/v12只安装G3.1，v15/v13只安装G3.2且继续blockmandatory bundle；v12 evaluation=v1、v13-v15=v2、v16=v3；parent-produced v12/v13与G0-G2生命周期必须通过 |
| mandatory dimensions / statuses | exact selected Solution `uses_ai`与subject/trigger lineage；固定六维exact order/cardinality；只允许`covered | insufficient_evidence | not_applicable`；source unavailable=`insufficient_evidence`；七类input ref/hash、ordinary readiness/conclusion ceiling、coverage summary、freshness、continuation、bundle status与consumer binding一致 |
| ceilings / report consumers | 七类specialized input与bundle状态采用最严格ceiling；complete只闭合coverage/freshness/mode，不消除specialized ceiling；prioritize要求exact bound + complete/not-required + aggregated `prioritize_allowed`及其他hard gate/panel/Portfolio/first-bet全部ready；missing/incomplete/desk-only/stale不得支持prioritize；comparison/recommendation/traceability/report及三个derived sidecar binding一致 |
| lifecycle / recovery | ordinary publication、checkpoint、clean reopen/recovery与report materialization通过；Manifest schema bundle推进到`15.0.0`；无Harness research/benchmark/network/LLM/agent dispatch、无external validation或伪造Evidence |
| NOT_RUN policy | required ordinary suite与required security mapping均必须逐项报告pass/fail/skip/todo；普通required `NOT_RUN`只可用于future environment明确缺少外部前置且有reason，不能替代repo-local验证。fixed-v7禁止的新identity/integrity/current-state/protected-data异常probe只能记`SECURITY_VALIDATION_NOT_RUN(reason)`，绝不能记PASS；`PD-G3-001`按C7永久保留finding/history并以`REMEDIATED_NON_BLOCKING`静态复核 |

#### G3.1-G3.3 Stage candidate preflight

全部最终验证使用Node.js `24.18.0`、npm `11.16.0`与TypeScript `7.0.2`。开工时默认shell曾解析到Node `26.5.0`/npm `11.17.0`，doctor按冻结contract正确返回fail；切换到`/opt/homebrew/opt/node@24/bin`后frozen doctor为PASS，错误runtime输出不作为candidate证据。

| 命令 / 边界 | Stage candidate结果 |
| --- | --- |
| `test:g3.1` / `test:g3.2` / `test:g3.3` | PASS；分别7/7、10/10、12/12；全部0 failed/cancelled/skipped/todo |
| `npm run lint` | PASS；Biome checked 343 files，0 fixes/errors |
| `npm run typecheck` | PASS；`tsc --noEmit`，0 errors |
| `npm test` | 最终PASS 321/321，0 failed/cancelled/skipped/todo；首次preflight为319/321，两个普通failure均是`tests/skeleton.test.ts`仍断言旧`skeletonVersion=g2.4`，更新为`g3.3`后skeleton专项8/8并完整重跑通过 |
| `npm run validate:schemas` | PASS；bundle `15.0.0`，161 schemas / 151 document validators，0 errors |
| `npm run validate:fixtures` | PASS 199/199，0 failed/cancelled/skipped/todo |
| `npm run verify:skeleton` | PASS；frozen doctor覆盖v16、bundle v15、publication v11、evaluation v3与G3.3入口 |
| `npm run validate:store` | PASS 11/11，0 failed/cancelled/skipped/todo |
| `npm run test:faults` | PASS 10/10 existing tests，0 failed/cancelled/skipped/todo；只复用仓库既有测试，未新增G3安全对抗probe |
| `npm run test:recovery` | PASS 11/11 existing tests，0 failed/cancelled/skipped/todo；只复用仓库既有测试，未新增G3安全对抗probe |
| G2/report/evaluation/traceability影响面 | `test:g2.4` PASS 32/32；完整`npm test`重放G0.4、G1.1-G1.4/report runtime、G2.1-G2.4、store/fault/recovery与G3.1-G3.3真实入口，0 skipped/todo |
| bytes / lineage / scope | accepted G2 -> `bed46a4` -> `4d727df` lineage正确；全部既有schema/policy JSON为0 modified/deleted，只新增v14-v16与对应versioned bundle/policy；`package-lock.json`、`.node-version`、`.npmrc`、`tsconfig.json`零差异；单一lockfile v3；`git diff --check` PASS；新增生产面无network/LLM/agent dispatch/external-validation执行入口 |
| ordinary / security NOT_RUN | required ordinary `NOT_RUN=0`。bound bundle/policy/receipt/Manifest identity/integrity/current-state与protected data-rights/privacy/trust异常分支按上文记录`SECURITY_VALIDATION_NOT_RUN(reason)`；mandatory-input-hash草稿probe单列protocol deviation，不计PASS或NOT_RUN |
...
- `PD-G3-001` classification=`SECURITY_SENSITIVE process blocker`，在第一次 acceptance 与 fixed-v5 authority 下的历史状态为`BLOCKING`。Finding ID、发生事实与当时authority永久保留；当前fixed-v7 C7处理结论见下一节，不保存、重述、复现或推导被禁止步骤。G3仍不得标`DONE`，G4仍为`NOT_READY`。
...
| ordinary / security NOT_RUN | required ordinary `NOT_RUN=0`；上述P1 identity异常分支与既有G3 bundle/policy/receipt/Manifest integrity/current-state、protected data-rights/privacy/trust异常分支继续逐项`SECURITY_VALIDATION_NOT_RUN(reason)`，不得记PASS；`PD-G3-001`在该历史candidate/fixed-v5下继续blocking，当前状态由fixed-v7 C7 supersede |
...
本 repair commit 使用账本占位避免self-reference，是 `b54fd76e341cd96c95642d30ecd21a47ea0685df` 的clean sole child `1e8969aced3376f8e4415ee6e32d75f4ae244a2b`。主worker preflight不是正式boundary PASS；该candidate的fixed-v5处理结论已由下一节fixed-v7 authority supersede。

#### G3 second whole-boundary FAIL fixed-v7 convergence repair

Fresh independent acceptance task `019fb12a-1c7b-7250-8f74-d190a213bbe4` 对 candidate `1e8969aced3376f8e4415ee6e32d75f4ae244a2b` 给出统一`FAIL`；这是G3第二次连续有效whole-boundary FAIL，count=`2`，因此按fixed-v7 C4进入`CONVERGENCE_REPAIR`。Acceptance的动态命令本身全部通过，新增`P1-G3-003` ordinary blocker指出v16 complete mandatory coverage错误覆盖了未闭合specialized input ceiling。本次repair不修改任何已发布schema/policy JSON bytes：`validateAiBundleContract`确定性归一化七类input ordinary readiness/显式ceiling并取最严格值，mandatory与consumer binding必须保存该exact aggregate；`aiBundleCompleteOrNotRequired`只在`bound + complete + prioritize_allowed`或合法non-AI `not_required`时为ready。

`createG33CompleteAiBundleFixture`现在显式闭合七类caller-supplied synthetic/unverified ordinary readiness与三个G3.2 ceiling，再闭合hard gates、panels、Portfolio与first bet。Fixture与测试只证明deterministic mechanics，不构成真实Evidence、成功benchmark、external validation、产品可行或市场成功。

##### fixed-v7 C4 invariant-to-test matrix

| Finding / classification | 永久 invariant | Authority / production source | 永久测试或existing-test mapping | 当前结论 |
| --- | --- | --- | --- | --- |
| `P1-G3-001` / ordinary | complete/not-required只解除missing/incomplete/desk-only/stale的AI degradation；仍受七类specialized ceiling及hard gate/panel/Portfolio/first-bet最严格ceiling | evaluation policy v3；RFC decision ceiling；`discovery-evaluation-validator.ts::aiBundleCompleteOrNotRequired`与recommendation ceiling | `G3.3 complete bundle permits...`正向；`complete AI readiness remains capped...`表驱动hard-gate/panel/Portfolio/first-bet；`incomplete, desk-only, and stale...`及既有missing测试 | repaired并与`P1-G3-003`共同闭合 |
| `P1-G3-003` / ordinary | v16 mandatory/consumer ceiling等于七类specialized input与bundle状态的最严格值；coverage complete不能清除economics/commoditization/trust或其他input的较低ceiling；prioritize显式要求aggregate=`prioritize_allowed` | evaluation policy v3 `mixed_inputs_use_strictest_ceiling`；RFC AI bundle/first-bet；`ai-bundle-validator.ts::specializedInputCeiling`、mandatory aggregate与consumer ceiling；`discovery-evaluation-validator.ts::aiBundleCompleteOrNotRequired` | `mandatory ceiling aggregates each specialized input...`覆盖七个单项与multi-input strictest；`complete coverage cannot override...`覆盖consumer/gate/recommendation；真正闭合positive fixture | repaired；required ordinary `NOT_RUN=0` |
| `P1-G3-002` / `SECURITY_SENSITIVE_VALIDATION` | exact matching mandatory bundle存在时consumer不得声明missing；bundle status/identity/consumer binding静态一致 | fixed-v7 C1/C5；`ai-bundle-validator.ts::validateAiBundleContract`的`matchingMandatoryPresent`、bound binding/lineage检查 | 只映射既有`G3.3`合法bound/missing/non-AI/report/publication/reopen测试、`store-faults`与`store-recovery`；没有新增identity/integrity/current-state动态负向probe | 静态production proof + existing-test mapping；异常分支`SECURITY_VALIDATION_NOT_RUN(reason=fixed-v7 permits static source and existing-test mapping only)`，不记PASS |
| `PD-G3-001` / `SECURITY_SENSITIVE process deviation` | finding ID、发生事实与authority永久保留；candidate不得依赖违规test/result，tracked/index/shared authority不得残留相关施工产物 | fixed-v7 C7；本账本G3.3 security mapping与candidate diff/status静态复核 | 无新增动态测试；只静态确认当前test/result矩阵无该项依赖，candidate/index/shared authority无残留 | `REMEDIATED_NON_BLOCKING`；不复现、推导或描述被禁止步骤 |

Ordinary mutation/combination coverage只改变caller-supplied业务状态：七类input逐项lower ceiling、multi-input `investigate_further_only + reject`取`reject`、complete specialized ceiling阻断prioritize、complete仍受四类first-bet requirement、以及missing/incomplete/desk-only/stale ceiling。没有为security-sensitive identity/integrity/current-state或protected data-rights/privacy/trust分支设计新动态变体。

fixed-v7 security mapping继续记录：bundle/policy/receipt/Manifest/Plan identity、integrity与persistent current-state异常分支为`SECURITY_VALIDATION_NOT_RUN(reason=fixed-v7 forbids new dynamic protected-authority or integrity probes; static source and existing tests only)`；protected data-rights/privacy/trust异常分支为`SECURITY_VALIDATION_NOT_RUN(reason=no protected authority-state mutation may be constructed in G3 convergence repair)`。`P1-G3-002`和这些分支不得记为动态PASS。

##### fixed-v7 C4 compliant shadow acceptance

全部shadow命令使用Node.js `24.18.0`、npm `11.16.0`、TypeScript `7.0.2`与唯一package-lock v3；所有H14-capable命令显式设置`STARTUP_OPPORTUNITY_H14_PARENT_GIT_SOURCE=/Users/chelaile/IdeaProjects/deep-search`。这是与正式whole-boundary同边界的主worker preflight，不是formal boundary PASS。

| 命令 / 边界 | Convergence repair shadow结果 |
| --- | --- |
| frozen doctor / `npm run verify:skeleton` | PASS；doctor `ok=true`、skeleton=`g3.3`、runtime=`24.18.0`、单一package-lock v3；0 fail |
| `npm run lint` / `npm run typecheck` | PASS；Biome checked 343 files、0 fixes/errors；`tsc --noEmit` 0 errors |
| `test:g3.1` / `test:g3.2` / `test:g3.3` | PASS 7/7、10/10、32/32；全部0 failed/cancelled/skipped/todo；G3.3新增七类single/multi-input、specialized ceiling、四类first-bet requirement与degraded-state ordinary coverage |
| `test:g2.4` + H14 exact producer/current | PASS 32/32，0 failed/cancelled/skipped/todo；exact parent builder bytes、parent/current lifecycle与forbidden sidecar recovery继续闭合 |
| `npm run validate:store` / `test:faults` / `test:recovery` | PASS existing tests 11/11、10/10、11/11；全部0 failed/cancelled/skipped/todo；fault/recovery仅作existing-test mapping，未新增security-sensitive probe |
| `npm test` | PASS 341/341，0 failed/cancelled/skipped/todo；完整重放G0-G3、report/store/fault/recovery与H14入口 |
| `npm run validate:fixtures` / `npm run validate:schemas` | PASS 219/219，0 failed/cancelled/skipped/todo；schema bundle `15.0.0`、161 schemas / 151 document validators、0 errors |
| v1-v16 dispatch / frozen bytes / scope | PASS；v12 evaluation v1、v13-v15 v2、v16 v3与receipt v10-v14 dispatch保持；相对`1e8969a`全部schema/policy JSON、package/toolchain bytes零修改；无network/research/LLM/agent/external-validation/G4/Workflow Runtime/DAG/daemon/UI/DB新增面；`git diff --check` PASS |
| ordinary / security NOT_RUN | required ordinary `NOT_RUN=0`；security-sensitive仅静态/existing-test mapping，上述异常分支继续`SECURITY_VALIDATION_NOT_RUN(reason)`，0新动态security probe |
| lineage / status / config | base=`1e8969a`、parent=`b54fd76`；repair candidate必须为exact sole child；tracked/index最终clean；用户自有untracked`.codex/config.toml`必须保持SHA-256=`131a40f8b7770196fa614f4267a71e030055c7708562f0b33284b66bfd0a2957`且不纳入candidate |

本convergence repair commit使用账本占位避免self-reference，必须是`1e8969aced3376f8e4415ee6e32d75f4ae244a2b`的clean sole child。完成后G3仍为`IN_PROGRESS`，G3.1-G3.3仍为`IMPLEMENTED_PENDING_WHOLE_GATE_REGRESSION`，G4=`NOT_READY`；不创建或建议acceptance。
...
### G4.1 Repo-local Skill / Agent / Hook / MCP Integration candidate

本切片以exact accepted G3 candidate `e00f71333e115a37cfcaf45590943e975d408ab4`为parent起点。`$startup-opportunity`现在明确执行discover/assess新Run、resume恢复与status只读路由；新增`status-run`只读Harness/Skill入口。三类custom agent保留typed envelope与唯一output ownership，lane researcher可记录/读取Evidence manifest，auditor/reviewer只能读取manifest；completion summary继续不是正式Artifact。

项目`.codex/config.toml`只注册repo-local stdio Evidence adapter并使用tool allow-list/approval mode；adapter仅接收caller-supplied bytes、调用既有Evidence Store和读取validated manifest，不fetch URL、不判断机会、不形成recommendation、不调用LLM/agent或接管Codex权限/thread lifecycle。Pre/Post/Stop hooks只提供可选机械guardrail/提示；不写正式`events.jsonl`，禁用后Skill与Harness显式步骤仍是完整正确性路径。

| 命令 / 边界 | G4.1 candidate结果 |
| --- | --- |
| `npm run test:g4.1` | PASS 4/4，0 failed/cancelled/skipped/todo；覆盖config/hooks/agent tool filter、ordinary hook继续、只读status、in-memory MCP caller-supplied Evidence交接 |
| frozen doctor | PASS；`ok=true`、skeleton=`g4.1`，新增config/hooks/MCP/status script均纳入repository contract |
| `npm run validate:store` / `npm run test:recovery` | PASS existing tests 11/11与11/11，全部0 failed/cancelled/skipped/todo |
| `npm run typecheck` | PASS；`tsc --noEmit` 0 errors |
| `npm test` | PASS 345/345，0 failed/cancelled/skipped/todo；完整重放G0-G4.1 |
...
### G4 fresh post-repair unified PASS and P1-G4-003 documentation repair

Fresh independent acceptance task `019fb221-9101-7d63-8f33-6bbc2740bd37` 对exact repair candidate `060029fbcfc6e4b543873642b7e3657c67c913af`给出unified `PASS`；final cursor=`cafe973d-042e-459d-a509-4be7e48156e4:3`，P0/P1/P2=`0/0/0`。因此G4.1-G4.3、G4.R、G4 Distribution / Operational Exit与W7在该accepted candidate上均为`DONE`。第一次有效boundary FAIL task `019fb1f3-9e4a-7d00-8239-28dcba107a08`、`P1-G4-001` ordinary repair、`P1-G4-002` static/existing-test mapping及其`SECURITY_VALIDATION_NOT_RUN(reason=...)`永久保留，不因后续PASS删除、弱化或扩张为security PASS。
