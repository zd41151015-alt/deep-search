...
- `$startup-opportunity` is a business Skill for executing Startup Opportunity research Runs. Do not invoke or follow it to guide repository exploration, debugging, coding, refactoring, schema design, code review, test strategy, or engineering task delegation. Coding agents must use this `AGENTS.md`, the owning production modules and current contracts, and the applicable engineering documentation instead. Inspect the Skill only when an engineering change directly modifies or verifies its Runtime entry behavior or public instructions; even then, it remains a consumer/documentation surface rather than an engineering authority.
