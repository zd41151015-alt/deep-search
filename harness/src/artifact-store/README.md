...
G0.4 defines the versioned adapter relationships still used by current Plan/Adaptation control artifacts: envelope v1 uses receipt v1, while envelope v2/v3 uses receipt v2 and matching Document Bundle reference rules. These adapters support current workflows inside a current-bundle Run; they do not promise old Run readability, install `future_declared` business schemas, or turn the Artifact Store into a migration layer.

G1.2 的 `research-publication.v1.json` 为 envelope v1-v5 发布唯一 adapter：receipt v1 对 v1，receipt v2 对 v2/v3，receipt v3 对 v4，receipt v4 对 v5。历史上“所有旧 schema/bundle/policy bytes 保持 immutable”的跨升级要求已被 current-only Run 策略 supersede；这里只保留当前 workflow 可达的 adapter tuple。v4 可发布已接受的 G1.1 Artifact，但明确阻止 `concept_evidence_assessment_branch_result.v1`；research branch 必须使用 v5 并经过 Evidence exact-record 与 typed traceability 校验。

G1.3 的 `research-publication.v2.json` 增加 envelope v6/document bundle v6/receipt v5 adapter，允许 immutable Gap Snapshot、Adaptation Decision 与 revision control Artifact publication。当前 Run 内由现行 workflow 写出的较早编号 receipt 与 adapter 仍可 recovery；跨版本旧 Run 不读取或恢复。recovery 必须验证 formal envelope bytes、canonical hash、operation identity 和 Manifest state，不能从 chat 或 task summary 重建。
...
Terminal reporting 的 `research-publication.v12.json` 在 immutable v11 policy 上增加 envelope v17/document bundle v17/receipt v15 adapter。v17 只发布 caller-supplied terminal source 及 Harness 确定性派生的 Decision Brief v3、terminal report view 和 Consistency v4；source/derived/materialized drift 均 fail closed，reopen 只从已验证 source 恢复。
