...
`record-evidence` 只接收调用方提供的 bytes，不抓取网页。`--url` 是当前可达的 v1 输入形式；`--source-url` 与 `--source-uri` 显式选择 v2。这些形式仅用于当前 bundle 创建的 Run，不承诺读取旧 Run。Reopen 会验证每条 JSONL record、receipt、raw hash、stable identity、唯一性和 exact fragment；完整损坏 fail closed，只有不完整尾部可截断。Store 成功不代表 Evidence 真实、独立、新鲜、有代表性或足以支持 thesis。
