...
默认 validator 使用 schema bundle `12.0.0`，同时保留既有 v1-v12 contract bytes。`validate-artifact` 继续只验证显式 document/bundle；不会扫描 Run、启动 Agent、执行 network research 或隐式迁移历史 Artifact。
