...
import {
  ...
  deriveSolutionEvaluationSummary,
} from "../validators/discovery-synthesis-validator.js";
...
function projectSolutionEvaluationSummaries(
  entries: readonly DraftedArtifact[],
  context: DiscoveryStageProjectionContext,
): void {
  const synthesisDocuments = new Map<string, DiscoverySynthesisDocument>();
  for (const [artifactPath, document] of context.documentsByPath) {
    synthesisDocuments.set(artifactPath, {
      path: artifactPath,
  ...
  for (const entry of entries) {
    synthesisDocuments.set(entry.path, {
      path: entry.path,
      schemaVersion: entry.type,
      document: entry.document,
      envelope: null,
    });
  }

  for (const opportunity of entries.filter(
    (entry) => entry.type === "startup_opportunity.opportunity_thesis.v1",
  )) {
    const evaluationRef = opportunity.document.solution_evaluation_ref;
    const evaluation =
      typeof evaluationRef === "string" ? synthesisDocuments.get(evaluationRef) : undefined;
    if (evaluation?.schemaVersion !== "startup_opportunity.solution_evaluation.v1") {
      throw new StoreError(
        "formal_materialization.solution_evaluation_summary_authority_invalid",
        "Opportunity solution summary projection requires an exact same-Run Solution Evaluation authority",
        { opportunityRef: opportunity.path, evaluationRef: evaluationRef ?? null },
      );
    }
    const expected = deriveSolutionEvaluationSummary(evaluation, synthesisDocuments);
    const supplied = opportunity.document.solution_evaluation_summary;
    if (
      supplied !== undefined &&
      (!isRecord(supplied) || canonicalJson(supplied) !== canonicalJson(expected))
    ) {
      throw new StoreError(
        "formal_materialization.solution_evaluation_summary_drift",
        "caller-supplied Opportunity solution summary drifts from the exact Solution Evaluation and formal Solution authorities",
        { opportunityRef: opportunity.path, evaluationRef },
      );
    }
    opportunity.document.solution_evaluation_summary = expected;
  }
}
...
export function projectDiscoverySynthesis(
  ...
  projectSolutionEvaluationSummaries(drafted, context);
  return compilerReady(drafted);
}
