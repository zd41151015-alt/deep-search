...
G3.1 默认加载 schema bundle `13.0.0`。既有 schema/policy bytes 保持 immutable；v14 Envelope/Document Bundle、receipt v12 与 publication policy v9只安装 caller-supplied capability、baseline benchmark、reliability 和 data dependency Artifact。Harness 只验证、聚合和持久化显式输入，不运行 research/benchmark，不生成语义判断，不访问 network，也不调用或派发 agent/LLM。
