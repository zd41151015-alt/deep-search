...
G2.4 默认加载 schema bundle `11.0.0`。已接受的 v9/bundle `8.0.0` contract、v10/bundle `9.0.0` G2.2 runtime 与 v11/bundle `10.0.0` G2.3 bytes 保持 immutable；G2.4 另用 v12 Envelope/Document Bundle、receipt v10、discovery evaluation policy v1 和 publication policy v7。Enrichment、Business Engine、hard gates、comparison、sensitivity、portfolio、recommendation、traceability 与 report 语义由调用方显式提供并经 deterministic validator/Store fail closed。
