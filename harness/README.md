...
G0.3 Store 保留原有 v1 envelope/receipt/document-bundle records，并提供 confined Run creation/reopen、canonical hashing、immutable formal publication、manifest indexing、Event/Decision append、Evidence substrate deduplication、checkpointing 和 crash recovery。Contract 修正默认发布 schema bundle `2.1.0` 与只读 Planning Contract evaluator；v2 envelope 仅可做显式 contract validation，G0.4 implementation 接通前不能经当前 Store publish。Plan/adaptation runtime 和 downstream research judgment 仍 fail-closed。
