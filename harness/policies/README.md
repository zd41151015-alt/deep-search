...
Every JSON file in this directory that is listed by `CURRENT_POLICY_PATHS` is an active policy document. Production loaders open those exact paths and validate them against `harness/schemas/current.json`; callers cannot select a historical or alternate policy path.
