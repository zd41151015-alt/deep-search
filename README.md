...
For Discovery G2.1 setup, dispatch waves, G2.2 fan-in, and G2.3 synthesis, Main Agent submits explicit research semantics to `materialize-formal-stage`: first `validate_only` to obtain a zero-write exact `publication_plan`, then `publish` with the same request and that exact plan. After G2.2 fan-in, the caller must show the exact current pre-candidates to the user and persist the caller-attested choice with `confirm-pre-candidates`. G2.3 may formalize only retained concrete pre-candidates selected by the latest same-fan-in `proceed_with_selected` confirmation; `run_additional_discovery_same_scope` goes back through the ordinary same-Scope Gap/adaptation path, and `stop_current_run` also covers scope mismatch before any new Run. The Harness derives paths, revisions, refs, hashes, Run/Plan/Task bindings, envelopes, and atomic publication order; it does not dispatch agents or infer research judgments. `compile-artifacts` remains the generic formal Artifact surface outside those stage-specific paths.
