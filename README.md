...
For Discovery G2.1 setup, dispatch waves, G2.2 fan-in, and G2.3 synthesis, Main Agent submits explicit research semantics to `materialize-formal-stage`: first `validate_only` to obtain a zero-write exact `publication_plan`, then `publish` with the same request and that exact plan. After G2.2 fan-in, the caller must show the exact current pre-candidates to the user and persist the caller-attested choice with `confirm-pre-candidates`. `follow-up-interest` is orthogonal to selected refs and can preserve interest in non-selected fan-in items for same-scope follow-up research without making them G2.3-eligible. The latest same-fan-in confirmation, ordered by append sequence rather than timestamp, is the current Manifest authority; `confirmedAt` is metadata only. G2.3 may formalize only retained concrete pre-candidates selected by the latest same-fan-in `proceed_with_selected` confirmation; `run_additional_discovery_same_scope` goes back through the ordinary same-Scope Gap/adaptation path, and `stop_current_run` also covers scope mismatch before any new Run. The Harness derives paths, revisions, refs, hashes, Run/Plan/Task bindings, envelopes, and atomic publication order; it does not dispatch agents or infer research judgments. `compile-artifacts` remains the generic formal Artifact surface outside those stage-specific paths.
...
Run data lives under `runs/<run_id>/` and is ignored by Git. `manifest.json` is the atomically replaced current index; plans, formal envelopes, checkpoints, and operation receipts are immutable. `create-run` appends an exact Scope proposal to `decisions.jsonl` and leaves the Run at `awaiting_scope_confirmation`. After that exact revision/ref/hash has been shown to the user, `confirm-scope` records only caller-attested confirmation; the Harness cannot authenticate chat identity and discloses that boundary. A correction uses `propose-scope` followed by a separate exact-bound `confirm-scope`; prior records are never replaced, and research remains blocked until the corrected Scope is reconciled through Gap/Decision/Plan. G2.2 pre-candidate interest is also durable `decisions.jsonl` state: the current Manifest authority is the latest same-fan-in append-ordered confirmation, not a timestamp, and `confirmedAt` is metadata only. Non-selection does not delete or reject Evidence, and scope mismatch is recorded as `stop_current_run` before any new Run. A user-authorized prior current-contract Run may enter only through `create-research-handoff`, which captures exact source bytes into the target Run and never inherits Plan, Task, Gate, ranking, execution, or terminal state. Discovery handoffs bind the current Plan; an Assessment pre-Plan handoff is Harness-marked and may only form the initial intake Concept before normal Plan publication. `read-research-handoff` reads only named items from that target-owned Artifact. If a terminal failed Run is repaired, the caller may create one fresh current-only attempt only with explicit technical restart provenance: exact source Run id, exact source Manifest hash, exact terminal report source ref/hash, and an explicit attestation. `status-run` may then show the source Run as `superseded_by_new_attempt` while the new Run stays current-only and inherits nothing. Reopen only through `load-run`, which validates durable state, repairs supported incomplete tails, reconciles completed operations, and fails closed on integrity conflicts without needing the source Run. A genuinely new market, language, decision scope, or later refresh uses a new current-contract Run. Terminal reporting always finishes on the original Run.
