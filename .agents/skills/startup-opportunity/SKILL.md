...
- 若已通过 `record_runtime_failure` 终止原 Run，修复后的新 Run 只能通过 caller-declared technical restart provenance 创建：精确 source Run id、source Manifest hash、source terminal report source ref/hash 和显式授权声明必须全部提供；不得从 Scope、chat、目录扫描或 run_id 形状猜测关系，新 Run 仍然是 current-only 且不继承 Plan、Task、结论或旧终态。
- 活动正式 Run 遇到需要修改 `harness/`、schema/policy、Skill/hook 或冻结工具链的 blocker 时，必须先通过 `record_runtime_failure` 在原 Run 生成 terminal report 并终止为 `failed`。不得修改生产代码后继续、恢复或重验同一 `run_id`；修复和工程验证完成后必须创建新的 `run_id`，而且旧 Run 只可被 status 解释为 `superseded_by_new_attempt`，不是同 Scope 关系或 chat 关系。
