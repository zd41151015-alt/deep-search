...
- Incumbent Response 只作 judgment context，不是 Gate，不自动淘汰或 unrank，不自动降低 Claim confidence，不产生 recommendation ceiling，也不阻塞 Lane、候选、Audit 或报告发布。Agent 对 `assessed` 提交完整语义；没有相关 responder 时只提交 `not_applicable` 与理由，缺 Evidence 时只提交 `unknown`、uncertainty、unknowns 和 data gaps，Harness 再扩展为统一安全报告行。`unknown` 必须进入 Search Closure 与报告 gap，`not_applicable` 不算 gap；每张正式响应风险表无论有无数据都固定声明 context-only/非门禁。新闻、评论、论坛、厂商、监管、API/数据集、proxy/estimate 以及 supporting/opposing/background Evidence 全部保留并标注角色；只有伪造、broken exact ref/hash、跨 Run、敏感信息、访问控制或错误引用继续 fail closed。
