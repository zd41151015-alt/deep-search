...
- Incumbent Response 只作 judgment context，不是 Gate，不自动淘汰或 unrank，不自动降低 Claim confidence，不产生 recommendation ceiling，也不阻塞 Lane、候选、Audit 或报告发布。没有相关 responder 时提交 `not_applicable`；缺 Evidence 时提交 `unknown`、uncertainty、limitations 和 data gaps。新闻、评论、论坛、厂商、监管、API/数据集、proxy/estimate 以及 supporting/opposing/background Evidence 全部保留并标注角色；只有伪造、broken exact ref/hash、跨 Run、敏感信息、访问控制或错误引用继续 fail closed。
