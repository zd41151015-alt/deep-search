...
- `status` 需要持久化 `run_id`；先读取 `references/artifact-contracts.md`，再运行只读 `scripts/status-run.ts`。只从 validated manifest 汇总状态，绝不启动 subagent、恢复或修改 Run。
...
- 综合、审计、比较和报告完成后，必须从同一 validated `report.json` 派生 `decision-brief.md` 与 `report.md`，完成 schema、traceability、freshness 和 consistency checks 后才交付。
