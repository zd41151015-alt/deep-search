...
Incumbent Response research 复用同一来源与 Evidence Store，不创建 provider/API 白名单或平行 Store。Execution Plan 每 stage 指定一个 owner 和可选显式 independent reviewer；所有状态（含 `not_assigned`）都必须完整投影到 Dispatch/Task/Audit，Task 不具备 fallback assignment 权威。对每个 assigned subject 先识别可能控制关键入口的 responder，再分别研究 ability/capability adjacency、implementation/operational/compliance/data/distribution cost、incentive 与 disincentive/cannibalization、plausible horizon、distribution leverage、response modes、可覆盖的 thesis 范围和 residual differentiation。支持、反对和背景 refs 分开保存；“能做”与“会做”、单一 feature 与完整 workflow/value proposition 必须保持为不同推理命题。Agent 的 `assessed` Delivery 到此为止，不编写行动、pass/fail、淘汰、推荐上限或正式 strategic implication；Harness 根据固定合同注入 reference-only context，validator 与报告只接受该固定边界。材料已存在但不足以完成判断时提交 `unknown`，并可选携带三类 Evidence role refs；Harness 保留这些 refs，同时把强风险维度安全展开为 unknown/空，统一说明提交材料与语义不足以形成完整结论，不声称没有 assessment 或 Evidence。可选 refs 与 assessed 一样接受 same-Audit Evidence Register、exact ref/hash、Run 与安全校验，不要求 adopted 才能作为 opposing/background。默认无 refs 的 `unknown` 仍正常进入 Search Closure/report gap；确实无相关控制点时提交最小 `not_applicable`，它不算 gap。两者正常发布，不构成 Gate 或自动决策副作用。
