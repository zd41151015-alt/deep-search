...
Incumbent Response research 复用同一来源与 Evidence Store，不创建 provider/API 白名单或平行 Store。Execution Plan 每 stage 指定一个 owner 和可选显式 independent reviewer；Dispatch/Task 只执行该投影。对每个 assigned subject 先识别可能控制关键入口的 responder，再分别研究 ability/capability adjacency、implementation/operational/compliance/data/distribution cost、incentive 与 disincentive/cannibalization、plausible horizon、distribution leverage、response modes、可覆盖的 thesis 范围和 residual differentiation。支持、反对和背景 refs 分开保存；“能做”与“会做”、单一 feature 与完整 workflow/value proposition 必须保持为不同推理命题。找不到材料时提交最小 `unknown`，它会进入 Search Closure/report gap；确实无相关控制点时提交最小 `not_applicable`，它不算 gap。两者正常发布，不构成 Gate 或自动决策副作用。
