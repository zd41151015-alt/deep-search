...
每个 branch 还要完成同一个 provider-agnostic `targeted_deep_dive` commercial audit：八个稳定量化 family、七类广义替代、exact raw Evidence acquisition provenance，以及明确 coverage gap。没有数据时显示 unavailable 和 decision impact，不以 proxy、新闻或估算冒充购买人数、付费人数、收入 observation 或市场验证。
