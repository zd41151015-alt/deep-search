...
Discovery report additionally shows each Opportunity's Solution exploration state and posture. All formal Solutions remain visible with their selected/alternative/rejected disposition; a non-compared state is rendered as the fixed provisional implementation posture, while compared states render the fixed compared selection posture. Considered but non-formalized approaches retain their reasons, exact material bindings, unknowns, limitations, and reachable same-Run task/subject lineage. Agent prose, Evidence, and source quotes remain admissible research material, but they do not prove, negate, or alter the structured solution posture.
...
失败、证据不足、取消或降优先级等非 completed terminal source 可以在 `research_provenance.captured_substrate_inventory` 中展示已采集但未正式化的 Evidence Store substrate。该投影由 Harness 从 same-Run exact `evidence/manifest.jsonl#ev_*` 记录机械派生，保留公开 URL 或用户提供 URI、research goal、`recorded_at`、未正式化原因和“不支持结论”边界。它是审计清单，不进入 `sources`、`report_citations`、正式 Evidence 采用/排除、覆盖、Gate、排序、置信度、建议上限、统计或 Evidence sufficiency；重复 raw/source identity 仍保留每个 exact record ref。

该清单的用户视图只能由 Harness-owned `captured_substrate_inventory` renderer 从结构化字段精确派生；Decision Brief 只展示有界摘要，完整报告、审计附录和结构化 `report.json` 保留全部 exact items。status、reason、conclusion boundary 和 source kind 使用字段专属穷尽映射，未知 structured 值在写入用户视图前失败。多行 research goal 保持 literal identity，但续行必须缩进在所属 Markdown 列表项内，不能冒充报告章节。中文用户视图校验按 surface 和结构化 source 绑定：审计附录的 Harness-owned Gate diagnostic 区必须与 `gate_warnings` 的本地化 row exact 闭合；Brief 和 core report 不扫描自由研究 prose。decision question、结论 prose、source claim/inference、commercial uncertainty、validation prose、freshness、limitations 和 inventory research goal 等自由研究 prose 不做关键词/路径 regex 扫描。不把任意 Markdown、caller prose、substring 位置、首个/末个 fragment 命中、字段白名单、全局 regex 或 source/citation 字面量豁免当作 authority。
