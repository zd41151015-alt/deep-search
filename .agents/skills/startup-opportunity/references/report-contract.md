...
Discovery 报告必须清楚分开三层：Scope 当前已知团队条件（硬约束、优势/短板、其他条件 `unknown`）、每个机会自身的五维启动负担、以及主 Agent 对该机会的匹配结论和未知前提。匹配结论只能是 `match`、`conditional`、`mismatch` 或 `unknown`；`unknown` 团队信息不得被渲染成无条件“适合当前团队”，也不得阻止机会保留和完整研究。Portfolio 的显式排序逐项绑定 comparison 与主 Agent 匹配分析，同时保留其他研究维度和限制；Harness 只机械投影，不计算匹配或排序。
