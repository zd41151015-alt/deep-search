...
排序使用 pairwise dominance、downside/upside 关系、speed to learn 和 partial order。面向用户的输出可以识别稳健领先组、近乎无法区分的候选，或指出 Evidence 不足以排序；Portfolio 的显式 `opportunity_ranking` 可以是并列、局部排序，或以 `rank: null` 明确保留未排序机会。绝不展示伪精确的全局分数、成功概率或被 Evidence 不足强迫出来的全序。
