...
Execution Plan 是 `incumbent_response_assignment` 唯一权威，并确定性投影到 Dispatch、Research Task 和 Audit。一个响应研究 stage 只能有一个 `owner`；需要复核时另设 `independent_review`，其他 Lane 明确 `not_assigned/none`，不得因同 stage kind 让所有 Lane 重复研究。candidate generation 只能 `not_assigned`；candidate evaluation 对 exact formed candidate refs 做 `lightweight_scan`；retained-candidate enrichment 和 assessment commercial stage 对 exact opportunity/concept refs 做 `targeted_deep_dive`。Lane 不做无界全候选深挖；`assessed` 一次性交付完整语义，`unknown/not_applicable` 使用最小输入，由 Harness 统一扩展。
