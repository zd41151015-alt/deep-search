...
同一 Execution Plan/Dispatch 还声明 `incumbent_response_assignment`。candidate generation 只能 `not_assigned`；candidate evaluation 对 exact formed candidate refs 做 `lightweight_scan`；retained-candidate enrichment 和 assessment commercial stage 对 exact opportunity/concept refs 做 `targeted_deep_dive`，其他 assessment stage 不得扩张该 scope。Lane 不做无界全候选深挖，一次性交付 assigned subject 的完整语义；Harness 对缺交付的已分配 subject 生成 `unknown`，而不是返工或失败。
