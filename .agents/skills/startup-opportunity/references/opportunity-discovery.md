...
`discover` 从宽泛的消费行业、用户群、场景或能力变化创建 `opportunity_discovery` Run。G2.1 只开放调用方显式提供的 Scope Frame、Seed Probe、Opportunity Space Map、Solution Space Map 和 Research Plan 的 closed validation/publication/reopen；`discover` orchestration、lane/fan-in 与正式 thesis 仍不可运行。
