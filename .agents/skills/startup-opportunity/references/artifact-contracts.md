...
当前 bundle `18.0.0` 的 v19 Assessment execution surface 包含 v2 Thesis、execution overlay、bounded dispatch、`assessment_evidence.v1`、lane result、受控 follow-up 和 stage gate。Assessment Evidence 必须绑定 exact `dispatch_batch.v2` task、当前 Plan/Thesis 以及 Evidence Store v2 substrate 的机械字段；lane result 只能消费通过该绑定验证的 Evidence。terminal early-kill gate 会把后续 unit 标为 `skipped` 并投影 Manifest 终态，replay/reopen 仍只消费同一当前 Run 内已验证的 immutable on-disk bytes。正式 termination apply 必须携带 caller-supplied main-agent source；Harness 不从 Manifest、chat 或 completion summary 生成结论。
