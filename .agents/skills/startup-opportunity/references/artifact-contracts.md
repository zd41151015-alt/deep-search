...
G2.2 增加 v10 envelope/document bundle、receipt v8、typed pre-thesis candidate、Research Task v2、lane material/result 与 reference-only fan-in runtime；accepted v9 contract envelope 继续不可写。G2.3 增加 v11 envelope/document bundle、receipt v9 和 executable conversion v2，不改写 v9/v10 bytes。Demand/Baseline/Solution/Opportunity Thesis、Solution Evaluation、pre-enrichment Snapshot 与 Merge 必须绑定 retained/current candidate ancestry、typed task material、generation/evaluation source separation 和 exact input closure；Harness 只发布 caller-supplied 语义，publication 不表示 Evidence 充分、validation 或市场成功。G2.4 artifact types 继续 fail closed。
