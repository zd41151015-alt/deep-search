...
Incumbent Response assignment 的 depth 只有 `not_assigned | lightweight_scan | targeted_deep_dive`，role 只有 `none | owner | independent_review`。Execution Plan 是唯一权威；每个响应研究 stage 恰有一个 owner，独立复核必须显式角色化，其余 Lane 为 `not_assigned/none`，Plan -> Dispatch -> Task -> Audit 必须 exact 投影。候选形成前必须 `not_assigned`；形成后的候选可做 lightweight scan，shortlist/retained opportunity 或 formed concept 的 commercial stage 可做 targeted deep dive。`assessed` 输入记录完整 responder 语义；`unknown` 输入只含 uncertainty/unknowns/data gaps，`not_applicable` 只含理由与可选 background refs，由 Harness 扩展成统一正式行。`unknown` 进入 Search Closure 和报告 gap，`not_applicable` 不形成 gap。该 Artifact 只作 judgment context，不进入 Gate、ranking eligibility、Claim confidence 或 recommendation ceiling 派生，也不阻塞发布。
