...
G2.2 增加 v10 envelope/document bundle、receipt v8、typed pre-thesis candidate、Research Task v2、lane material/result 与 reference-only fan-in runtime；accepted v9 contract envelope 继续不可写。G2.3 增加 v11 envelope/document bundle、receipt v9 和 executable conversion v2，不改写 v9/v10 bytes。Demand/Baseline/Solution/Opportunity Thesis、Solution Evaluation、pre-enrichment Snapshot 与 Merge 必须绑定 retained/current candidate ancestry、typed task material、generation/evaluation source separation 和 exact input closure。G2.4 增加 v12 envelope/document bundle、receipt v10、Research Task/material v3、enrichment branch/fan-in、domain enrichment、comparison/sensitivity/portfolio/recommendation/traceability 和 discovery report；Research Task v3 必须匹配 current immutable Plan 的 exact enabled unit tuple，不能只引用 Plan path。v11 继续在写入前 block 这些 types，v12 继续 block G3 AI bundle。Harness 只发布 caller-supplied 语义并确定性派生 report views，publication/report success 不表示 Evidence 充分、validation 或市场成功。
