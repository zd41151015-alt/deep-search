...
候选或 concept 形成后，系统才研究 Incumbent Absorption & Response Risk。solution-neutral candidate generation 不得出现该分析；formed candidate evaluation 可做 bounded `lightweight_scan`，shortlist/retained opportunity 或 assessment commercial stage 才可分配 `targeted_deep_dive`。Execution Plan 是 assignment 唯一权威：每个响应研究 stage 恰有一个 `owner`，独立复核必须显式标为 `independent_review`，其他 Lane 为 `not_assigned/none`；所有 assignment 状态（包括 `not_assigned`）都必须按 Plan -> Dispatch -> Research Task -> Audit exact subject/role/depth 投影，缺少任一层或发生漂移均为 integrity error，不能回退由 Task 决定“不调研”。不得对所有候选执行无界深挖。

潜在 responder 不限同类大公司，还包括 platform owner、suite incumbent、adjacent leader、channel/distribution controller、data owner、marketplace 或其他控制关键入口的主体。每个 Agent 判断分别记录 responder/category/control point、开放 response modes（例如 copy、bundle、native integration、API/platform move、pricing/subsidy、acquire、restrict/block）、capability adjacency、implementation/operational/compliance/data/distribution cost、incentive/disincentive/cannibalization、plausible response horizon、distribution leverage、single-feature/partial-workflow/full-value-proposition thesis coverage、residual differentiation、supporting/opposing/background Evidence、inference boundary、confidence/uncertainty 和 unknown/data gap。Agent Delivery 不包含 pass/fail、淘汰、行动、ranking/Claim/ceiling/publication 指令或正式 `strategic_implication`；Harness compiler 为所有正式状态固定注入 reference-only strategic context，Schema、商业 validator 与报告 projection/renderer 共同强制该决策边界。

Ability 不能推导 incentive、时间或必然响应；复制 feature 不能推导完整 thesis 已被覆盖。该 Artifact 只作 aggregate/synthesis 的 judgment context，不是 Gate，不自动淘汰或 unrank，不自动降低 Claim confidence，不产生 recommendation ceiling，也不阻止 Lane、候选、Audit 或报告发布。`assessed` 使用完整的结构化研究输入；缺少足够材料形成完整判断时 Agent 提交 `unknown` 的 uncertainty/unknowns/data gaps，并可选携带 supporting/opposing/background Evidence refs；确实无相关 responder 时只提交 `not_applicable` 理由。Harness 保留 `unknown` 的三类 refs，同时把 ability、cost、incentive、horizon、thesis coverage、residual differentiation、confidence 等强风险维度统一安全展开为 unknown/空。explicit unknown 与自动补齐 unknown 使用相同的中性 rationale：提交材料与 assessment 语义不足以形成完整 responder-specific conclusion；它不声称没有 assessment、没有搜索或没有 Evidence。可选 refs 继续接受 same-Audit Evidence Register、exact ref/hash、Run 和安全校验，不要求 adopted 才能作为 opposing/background。默认无 refs 的 `unknown` 仍正常发布。`unknown` 必须进入 Search Closure 与正式报告 gap，`not_applicable` 不算 gap；每个正式响应风险表固定显示 context-only/非门禁声明。新闻、评论、论坛、厂商、监管、API/数据集、proxy/estimate 及支持/反对/背景材料继续 preserve first, weight later，不设置来源、provider 或 API 白名单；伪造、broken exact ref/hash、跨 Run、敏感信息、访问控制和错误引用仍 fail closed。
...
`decision_tier` 受 comparison、enrichment fan-in、候选级 commercial aggregate 和 first-bet readiness 的 closed ceiling 共同约束。`recommended_first_bet` 非空时只读取该 subject 的 ceiling；`alternative_bets`、watchlist 和其他展示候选不参与 first-bet ceiling。`prioritize` 只允许 first bet 与 Portfolio exact 一致、其 comparison 为 `strong_candidate`/`eligible`、全部 hard gate 为 `passed | not_applicable`、fan-in conclusion ceiling 为 `strong_candidate`、四个 panel 均 `sufficient` 且不为 `weak | unknown`、AI mandatory bundle 已完整或确实不适用，并且该 subject 的 aggregate commercial ceiling 为 `prioritize`。切换 first bet 后必须重新读取新 subject 的全部 ceiling。

每个候选的 `recommendation_band` 同时受自己的 aggregate commercial ceiling 约束；弱 alternative 不得被描述为超过自身 Evidence ceiling 的强候选。`recommended_first_bet=null` 时禁止 `prioritize`，并按全部候选中最佳的直接 readiness 得到最高允许 tier：至少一个候选达到 investigate readiness 时为 `investigate_further`，否则至少一个达到 watch readiness 时为 `watch`，都没有时为 `insufficient_evidence`。这不是取展示集合中最弱候选，也不由 Harness 生成推荐语义；Harness 只确定性验证 caller 的 tier 未超过该 portfolio readiness。
...
量化和竞品 Lane 覆盖由 Dispatch 驱动。每个 Lane 只闭合明确分配的 commercial dimensions、metric families 和 competitor types；定性 Lane 可分配空的量化/竞品数组。Lane-local uncovered dimensions、ranking eligibility 和 ceiling 只诊断该 Dispatch 分配范围，不能直接成为候选终态。`partial | unavailable | not_applicable` 是诚实、可发布的状态。八类量化和七类替代的整体完整性只在候选、Wave 或终态聚合时评估，不能要求每个 Lane 填满。来源集中、vendor-only 和缺少独立交叉验证产生 warning，并确定性降低 Claim confidence、ranking eligibility 或 disposition/recommendation ceiling，不能仅显示文字后继续给出强结论。

Harness 按 subject 聚合全部 current、有效 Audit 的 bound Evidence、business coverage、量化/竞品 coverage、Wave 1 signals、反证、冲突、limitations 和 Gaps，再派生候选级 `commercial_subject_aggregates`、ranking eligibility 与 recommendation ceiling。一个维度只要被任一 current Audit 合格关闭，就不因另一重复或未分配 Lane 缺失而再次形成候选级 coverage gap；但冲突 Evidence、来源集中和不一致必须继续进入 aggregate confidence/ceiling，不能以“最佳 Lane 状态”抹掉。Lane ceiling 可以保留为诊断，终态消费者不得对全部 Lane ceiling 简单取最严格值。

同一 `evidence_ref` 在多个 current Lane 中的登记是多份研究解释，不是可以按 Audit 路径覆盖的重复行。来源集中度、ref closure 和 provenance 计数按 canonical ref 去重；counterevidence、support/context、adopted/rejected disposition 和 limitations 则保留全部 Lane interpretation。任一 Lane adopted 的反证继续限制 subject ceiling；涉及至少一个 adopted interpretation 的关键 support/counter 或 disposition 冲突同样限制 ceiling。rejected-only 反证或 all-rejected disagreement 继续完整保留并形成适用的非阻塞 warning/aggregate limitation，但不自动获得决策权重或进入 active conflict refs。Audit 文件名或输入顺序不得改变 conflict refs、ceiling 或聚合语义。

Incumbent Response 与这些强度派生隔离。Execution Plan 是每个 `assigned | not_assigned` assignment 的唯一权威，Dispatch、Task 和 Audit 必须完整确定性投影；缺失 authority 或任一漂移都是 integrity error，不允许 Task fallback。Plan/Dispatch 只为候选后的 exact subjects 分配 `lightweight_scan | targeted_deep_dive`，Lane 一次性交付完整结构化研究维度，Harness 生成稳定 assessment ID/ref/hash、固定 reference-only strategic context 和 `unknown` coverage。Delivery Schema 不接受 Agent authored strategic action，Compiler 不复制额外决策文本，正式 Schema 与商业 validator 拒绝 strategic context 漂移，报告 projection/renderer 也只输出固定边界。`unknown` 可选保留三类 Evidence role refs，但强风险维度保持 unknown/空；这些 refs 继续受同 Audit Register、exact ref/hash、Run 和安全校验。Response assessment 可以被 synthesis 引用，但 policy 固定其 automatic effects 为 Gate/ranking eligibility/Claim confidence/recommendation ceiling/artifact publication 全部 `false`；任何影响排序或建议的使用都必须来自显式、可解释且受其他正式合同约束的 synthesis，而不是 Response Artifact 或 deterministic validator。

Agent-authored commercial delivery 只包含研究语义：研究目标、主要 route、正式记录的搜索结果、Evidence 来源与内容、Finding/Claim/Judgment、量化与竞品 observation、非决策性的 Incumbent Response 结构化维度、未解决 Gap、limitations、停止原因及 telemetry 声明。Harness compiler 确定性生成正式 `commercial_research_audit` 的 ID、revision/hash/refs、`derived_valid_as_of`、freshness、adopted source distribution、business/coverage closure、Search/Evidence reconciliation、confidence/ranking、recommendation ceiling、固定 response decision boundary 和 report rows。报告中的 quantitative rows、competitive rows、incumbent response rows 和 coverage gaps 只从正式 audit 投影；caller 副本会被覆盖，projection mismatch 只表示 Harness 内部不变量失败。
...
多候选 task 的 `covered_direction_ids`、`target_opportunity_refs` 或 `target_candidate_refs` 只声明研究范围，不授权 Evidence 在 subjects 间串用。单候选 task 可自动绑定全部 Evidence；多候选 task 优先从 Finding/Claim/Judgment、量化 observation、competitive observation 及其 refs 确定性派生 subject binding。真正跨候选共享的市场级 Evidence 可以显式列出多个 `subject_ids`。无法绑定的材料继续保留为 `commercial_background_material` 并产生 warning，但不关闭任何候选的 direct coverage，也不支持其 Claim confidence 或 ranking。Targeted deep dive 可以偏好一候选一 task；broad scan 和横向比较仍允许多候选 Audit。

`search_closure.remaining_gaps` 是 compiler 派生的 structured current rows：Delivery 只提供 coverage kind/dimension、研究原因、decision impact、尝试和可选 subject 研究语义；Harness 根据单 subject task、显式共享 subject 或多候选无法绑定状态补齐 `subject_ids`、binding basis、exact task ref 与 Audit ref。Business/research gap 只进入其绑定 subject 的 aggregate 与正式 gap table；显式共享 gap 可以进入多个 subject，无法绑定的 gap 只作为 portfolio research context 保留。另一 Audit 已直接关闭同一 business dimension 时，不重复形成 subject gap。

计划 task 缺少 current Audit 时不阻塞报告。Harness 从 task/Dispatch target、`required_commercial_dimensions`、assigned metric families/competitor types 和 current lifecycle 状态生成 `unavailable` execution/research gap；不伪造 query、Observation 或数值。量化/竞品缺口使用正式 coverage row，无法进一步映射的缺失保留 task-level execution row。报告明确区分 `complete | planned_with_gaps | planned_but_missing | not_planned`；空 gap 数组只有在 `complete` 时才能渲染为全部已观察。缺失 task 的分配维度若已被另一 current Audit 关闭，只保留 execution warning；未被任何 Audit 关闭时才降低对应 subject ceiling。另一 subject 的缺失不影响 first bet。

Recommendation ceiling 按报告实际决策对象绑定。Discovery 的主 tier 只使用 `recommended_first_bet`，Concept Assessment 使用 concept hypothesis；Terminal opportunity discovery 使用唯一 `priority=1` direction，或没有排名主项时的唯一 `invest` direction。其他 validate/defer/reject direction 只作为展示和 portfolio risk，不压低 primary，但每个 direction 的 action 仍不得超过自身 ceiling。没有明确 Terminal primary 时禁止 `prioritize`，按最佳候选 readiness（最高 `investigate_further`）校验主结论，而不是取最弱展示候选。`top_opportunity_refs`、alternative、watchlist、deprioritized 或 rejected 对象不能让任何对象绕过自身 ceiling。Claim confidence 只受其 Evidence refs、claim type/coverage keys、freshness、independence、direct/inference、冲突材料和相关维度 Gap 影响；例如 retention Gap 不降低由当前直接 Evidence 支持的公开价格 Claim，但整体 ranking 和 recommendation 仍受决策相关 Gap 限制。
...
Evaluator 解析 selected Solution `uses_ai` 并强制 missing-G3 AI ceiling。每个候选先独立计算 fan-in、comparison、hard gates、panels 和 subject commercial aggregate ceiling；Recommendation 有 first bet 时只取该 subject 的 closed tier，无 first bet 时使用明确的最佳候选 portfolio readiness，绝不让最弱展示候选决定主 tier。`build-report` 只从 validated `report.v1` 确定性派生 Decision Brief v2、Discovery Report View 和 Consistency Evaluation v3，再以独立 current receipt materialize 三个固定 view；每个表面在 publication、checkpoint/reopen 和 recovery 中重验禁用表述、refs 与 hashes。任何 schema、Store、summary 或 report success 都不表示 Evidence 真实/充分、市场验证或商业成功。
...
- Fixture 覆盖候选形成前 `not_assigned`、formed candidate lightweight scan、shortlist/concept targeted deep dive、所有状态的 Plan -> Dispatch -> Task -> Audit 完整投影与漂移反例、带/不带三类 Evidence role refs 的 unknown、not_applicable 正常发布、多候选 subject 隔离、ability 高但 incentive 低且只覆盖单一 feature 的反例、broken ref fail closed、高风险不自动 unrank/降低 Claim confidence/触发 recommendation ceiling/阻塞发布，以及 action-bearing assessed Delivery 被拒绝且无法进入正式 Audit/三类报告、strategic context 篡改被 Schema 与 validator 拒绝、unknown rationale 不虚构未提交 assessment 或没有 Evidence、report consistency/replay 保持固定边界。
