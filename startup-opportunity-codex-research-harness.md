...
Ability 不能推导 incentive、时间或必然响应；复制 feature 不能推导完整 thesis 已被覆盖。该 Artifact 只作 aggregate/synthesis 的 judgment context，不是 Gate，不自动淘汰或 unrank，不自动降低 Claim confidence，不产生 recommendation ceiling，也不阻止 Lane、候选、Audit 或报告发布。`assessed` 使用完整输入；缺少足够材料形成完整判断时 Agent 提交 `unknown` 的 uncertainty/unknowns/data gaps，并可选携带 supporting/opposing/background Evidence refs；确实无相关 responder 时只提交 `not_applicable` 理由。Harness 保留 `unknown` 的三类 refs，同时把 ability、cost、incentive、horizon、thesis coverage、residual differentiation、confidence 等强风险维度统一安全展开为 unknown/空。可选 refs 继续接受 same-Audit Evidence Register、exact ref/hash、Run 和安全校验，不要求 adopted 才能作为 opposing/background。默认无 refs 的 `unknown` 仍正常发布，并只说明没有提交足以完成 assessment 的 Evidence-role binding/语义，不能声称 Evidence Register 中没有材料。`unknown` 必须进入 Search Closure 与正式报告 gap，`not_applicable` 不算 gap；每个正式响应风险表固定显示 context-only/非门禁声明。新闻、评论、论坛、厂商、监管、API/数据集、proxy/estimate 及支持/反对/背景材料继续 preserve first, weight later，不设置来源、provider 或 API 白名单；伪造、broken exact ref/hash、跨 Run、敏感信息、访问控制和错误引用仍 fail closed。
...
- Fixture 覆盖候选形成前 `not_assigned`、formed candidate lightweight scan、shortlist/concept targeted deep dive、所有状态的 Plan -> Dispatch -> Task -> Audit 完整投影与漂移反例、带/不带三类 Evidence role refs 的 unknown、not_applicable 正常发布、多候选 subject 隔离、ability 高但 incentive 低且只覆盖单一 feature 的反例、broken ref fail closed，以及高风险不自动 unrank、降低 Claim confidence、触发 recommendation ceiling 或阻塞发布。
