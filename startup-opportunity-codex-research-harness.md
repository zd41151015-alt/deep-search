...
同一 `evidence_ref` 在多个 current Lane 中的登记是多份研究解释，不是可以按 Audit 路径覆盖的重复行。来源集中度、ref closure 和 provenance 计数按 canonical ref 去重；counterevidence、support/context、adopted/rejected disposition 和 limitations 则保留全部 Lane interpretation。任一 Lane 识别出的反证继续限制 subject ceiling；相互矛盾的 interpretation 形成非阻塞、可见的 warning 与 aggregate limitation，Audit 文件名或输入顺序不得改变 conflict refs、ceiling 或聚合语义。
