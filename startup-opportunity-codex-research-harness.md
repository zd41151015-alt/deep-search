...
Recommendation ceiling 按报告实际决策对象绑定。Discovery 的主 tier 只使用 `recommended_first_bet`，Concept Assessment 使用 concept hypothesis，Terminal 使用当前被推荐/验证的 direction；`top_opportunity_refs`、alternative、watchlist、deprioritized 或 rejected 对象只作为展示和 portfolio risk，不压低无关对象，也不能让任何对象绕过自身 ceiling。Claim confidence 只受其 Evidence refs、claim type/coverage keys、freshness、independence、direct/inference、冲突材料和相关维度 Gap 影响；例如 retention Gap 不降低由当前直接 Evidence 支持的公开价格 Claim，但整体 ranking 和 recommendation 仍受决策相关 Gap 限制。
