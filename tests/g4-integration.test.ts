...
test("prior Run semantics require exact admission before Agent reads and cannot use generic Decision append", async (context) => {
  ...
  const reinterpreterSensitiveCommands = [
    ...
    [`(( value = 1 << 2 ))`, `bash -c 'cat "$PRIOR_ARTIFACT_PATH"'`].join("\n"),
    `command exec -a research bash -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `builtin exec -a research bash -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `command builtin eval 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `builtin command eval 'cat "$PRIOR_ARTIFACT_PATH"'`,
  ...
  const reinterpreterAllowedCommands = [
    ...
    `command exec -a research bash -c 'cat "$PRIOR_RESPONSE_REF"'`,
    `command builtin printf '%s' '$PRIOR_ARTIFACT_PATH'`,
    `command -v bash -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `command -V bash -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
  ];
  ...
  for (const command of [
    ...
    ...reinterpreterSensitiveCommands,
