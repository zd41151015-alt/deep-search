...
test("prior Run semantics require exact admission before Agent reads and cannot use generic Decision append", async (context) => {
  ...
  const reinterpretedSensitiveCommands = [
    `bash -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `/bin/sh -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `/bin/zsh -lc 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `eval 'cat "$PRIOR_ARTIFACT_PATH"'`,
    [`bash <<'SCRIPT'`, `cat "$PRIOR_ARTIFACT_PATH"`, "SCRIPT"].join("\n"),
    [`bash -c 'cat "$PRIOR_ARTIFACT_PATH"`, `'`].join("\n"),
    ...
    `builtin eval 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `printf '%s' "$(bash -c 'cat "$PRIOR_ARTIFACT_PATH"')"`,
    `printf '%s' "\`bash -c 'cat \\"$PRIOR_ARTIFACT_PATH\\"'\`"`,
    `printf '%s\\n' 'cat "$PRIOR_ARTIFACT_PATH"' | bash`,
    `bash <<< 'cat "$PRIOR_ARTIFACT_PATH"'`,
    `if true; then bash -c 'cat "$PRIOR_ARTIFACT_PATH"'; fi`,
    `! /bin/sh -c 'cat "$PRIOR_ARTIFACT_PATH"'`,
    [`cat <<SCRIPT`, `$(bash -c 'cat "$PRIOR_ARTIFACT_PATH"')`, "SCRIPT"].join("\n"),
    [`bash -c 'cat "$PRIOR_\\`, `ARTIFACT_PATH"'`].join("\n"),
    [`bash <<'SCRIPT'`, `cat "$PRIOR_\\`, `ARTIFACT_PATH"`, "SCRIPT"].join("\n"),
    [`cat <<'SCRIPT' | bash`, `cat "$PRIOR_ARTIFACT_PATH"`, "SCRIPT"].join("\n"),
  ];
  const reinterpretedAllowedCommands = [
    `bash -c 'cat "$PREVIOUS_API_RESPONSE_FILE"'`,
    `eval 'cat "$PRIOR_QUERY_RESULT_PATH"'`,
    [`zsh <<'SCRIPT'`, `cat "$PREVIOUS_QUERY_STAGING_DIR"`, "SCRIPT"].join("\n"),
    `command /bin/sh -c 'printf child-ok'`,
    [`printf '%s' 'literal`, `$PRIOR_ARTIFACT_PATH'`].join("\n"),
  ...
  for (const command of [
    ...
    ...reinterpretedSensitiveCommands,
