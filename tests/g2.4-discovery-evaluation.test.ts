...
test("G2.4 validates closed enrichment, hard gates, comparison, portfolio, and report lineage", async (context) => {
  ...
  const fixtureTasks = [G24_TASK_SUPPORT, G24_TASK_CHALLENGE].map((taskRef) =>
    effective(state.bundle, taskRef),
  );
  assert.equal(fixtureTasks.length, 2);
  assert.ok(
    fixtureTasks.every((task) =>
      [G23_OPPORTUNITY_A, G23_OPPORTUNITY_B].every((ref) =>
        (task.target_opportunity_refs as string[]).includes(ref),
      ),
    ),
  );
  assert.equal(
...
test("G2.4 rejects closed contract mutations with deterministic error codes", async (context) => {
  ...
  const mutations: readonly {
    ...
    },
    {
      ...
      mutate(bundle) {
        const gates = effective(bundle, G24_FAN_IN).hard_gate_inputs as Record<string, unknown>[];
        gates.push(clone(gates[0] as Record<string, unknown>));
        refresh(bundle, G24_FAN_IN);
      },
    },
    {
