...
test("published schema bundle is closed, versioned, and internally resolvable", async () => {
  ...
  assert.equal(result.schemaBundleVersion, "11.0.0");
  assert.equal(result.schemaCount, 135);
  assert.equal(result.documentSchemaCount, 127);
...
test("G2.2 Scheme A bundle installs a closed pre-thesis candidate contract", async () => {
  ...
  assert.equal(schema.schemaBundleVersion, "11.0.0");
  assert.equal(schema.schemaCount, 135);
  assert.equal(schema.documentSchemaCount, 127);
...
test("v9 remains validation-only while v10 owns G2.2 runtime publication", async () => {
  ...
  assert.equal(result.valid, true, JSON.stringify(result.errors));
  assert.equal(validator.publicationPolicy.document.current_schema_bundle_version, "11.0.0");
  assert.equal(validator.publicationPolicy.document.adapters.length, 11);
  assert.equal(
