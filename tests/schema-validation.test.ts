...
test("published schema bundle is closed, versioned, and internally resolvable", async () => {
  ...
  assert.equal(result.schemaCount, 108);
  assert.equal(result.documentSchemaCount, 101);
...
test("G2.2 Scheme A bundle installs a closed pre-thesis candidate contract", async () => {
  ...
  assert.equal(schema.schemaBundleVersion, "10.0.0");
  assert.equal(schema.schemaCount, 108);
  assert.equal(schema.documentSchemaCount, 101);
...
test("v9 remains validation-only while v10 owns G2.2 runtime publication", async () => {
  ...
  assert.equal(result.valid, true, JSON.stringify(result.errors));
  assert.equal(validator.publicationPolicy.document.current_schema_bundle_version, "10.0.0");
  assert.equal(validator.publicationPolicy.document.adapters.length, 10);
  assert.equal(
